...
root=Path('results/marginal_h6/polynomial_metric/bare_joint');prepared=root/'search';out=root/'native_ipm_extended';out.mkdir(exist_ok=True);data=json.loads((root/'hamiltonian.json').read_text());meta=json.loads((prepared/'prepared.json').read_text());assert input_digest(data)==meta['source_sha256'];matrix=load_npz(prepared/'model.npz');rhs=np.load(prepared/'rhs.npy',allow_pickle=False);rhs[:(len(rhs)-1)//2]*=.9;orbits=meta['orbits'];labels=meta['labels'];floor=F(1,10000);n=len(orbits);a=len(labels);assert matrix.shape==(len(rhs),n+2*a+1)
...
for key,value in [('output_flag',True),('threads',1),('solver','ipm'),('simplex_strategy',1),('presolve','on'),('time_limit',360.),('primal_feasibility_tolerance',1e-8),('dual_feasibility_tolerance',1e-8)]:
...
if solution.value_valid:
 ...
 if x[-1]>0:
  proposal={'source_sha256':input_digest(data),'orbits':orbits,'metric_coefficients':x[:n].tolist(),'metric_bound':.0009,'numerator_bound':float(x[-1]),'gamma':meta['gamma']}
