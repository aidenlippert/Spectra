...
out=Path('/tmp/audit_polynomial_full_dual');out.mkdir(exist_ok=True)
...
sector=spin_states(SpinZeroOracle(data))
...
for mask in masks:
 ...
 for assignment in range(1<<len(bits)):
  ...
  if len(bits)<=4 or (len(bits)<=degree and __import__('math').comb(6-sum(bool(mask&(1<<i)) for i in range(0,12,2)),3-sum(bool(occ&(1<<i)) for i in range(0,12,2)))*__import__('math').comb(6-sum(bool(mask&(1<<i)) for i in range(1,12,2)),3-sum(bool(occ&(1<<i)) for i in range(1,12,2))) not in (0,1)) :
...
rows=sector; ri=[];ci=[];values=[]
for j,col in enumerate(columns):
 for i,state in enumerate(rows):
  value=sum(v for mask,v in col.items() if mask&state==mask)
  if value: ri.append(i);ci.append(j);values.append(value)
a=csc_matrix((np.asarray(values,dtype=float),(ri,ci)),shape=(len(rows),len(columns)));objective=np.array([-1 if label[0]=='b' else 0 for label in labels],dtype=float)
...
for name,scale in [('numerator',compiled['receipt']['numerator_scale'])]:
 poly={mask:int(value)/scale for mask,value in compiled[name]}; poly={state:sum(v for mask,v in poly.items() if mask&state==mask) for state in rows};bounds[0]=((0.01,0.01) if name=='weight' else (0.001,0.001));start=time.monotonic();r=linprog(objective,A_eq=a,b_eq=[poly.get(m,0) for m in rows],bounds=bounds,method='highs-ipm',options={'time_limit':120})
