...
root=Path('/Users/aidenlippert/Documents/Spectra/results/marginal_h6/polynomial_metric');out=root/'localized_quadratic_sos';out.mkdir(exist_ok=True)
...
for j in range(a.shape[1]):
 lo,hi=a.indptr[j:j+2];key=(tuple(a.indices[lo:hi]),tuple(a.data[lo:hi]))
 if key not in seen:seen.add(key);keep.append(j)
a=a[:,keep];labels=[labels[j] for j in keep]
print(json.dumps({'phase':'deduplicate','columns':a.shape[1]}),flush=True)
for degree in [2]:
 ...
 z=cp.Variable(len(states));nonnegative=a.T@z>=0;normal=cp.sum(z)==1;psd=f.T@cp.diag(z)@f>>0;charge_psd=f.T@cp.diag(cp.multiply(d,z))@f>>0;prob=cp.Problem(cp.Minimize(p@z),[normal,nonnegative,psd,charge_psd]);start=time.monotonic();print(json.dumps({'phase':'solve','degree':degree,'rank':rank}),flush=True)
 ...
 receipt={'degree':degree,'rank':rank,'status':prob.status,'value':prob.value,'seconds':time.monotonic()-start,'scope':'Numerical quadratic SOS and (D-1)-localized quadratic SOS plus nonsingleton indicator/localizer dual on explicit400-sector coordinates. QR rank selection is numerical discovery only.'}
 if z.value is not None:
  ...
  proposal={'states':states,'basis':basis,'dual_weights':z.value.tolist(),'bound':float(-normal.dual_value),'atom_labels':labels,'atom_weights':nonnegative.dual_value.tolist(),'gram':psd.dual_value.tolist(),'charge_gram':charge_psd.dual_value.tolist()}
