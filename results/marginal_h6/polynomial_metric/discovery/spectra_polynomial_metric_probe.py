...
out=root/'results/marginal_h6/polynomial_metric';out.mkdir(exist_ok=True)
for degree in [2,3,4,5,6]:
 orbits={}
 for powers in product(range(3),repeat=m):
  if not 1<=sum(powers)<=degree:continue
  canonical=min(powers,powers[::-1]);orbits.setdefault(canonical,[]).append(powers)
 orbits=list(orbits.values());f=np.column_stack([sum(np.prod(charges**np.array(powers),axis=1) for powers in orbit) for orbit in orbits]).astype(float);k=a@f;n=f.shape[1]
