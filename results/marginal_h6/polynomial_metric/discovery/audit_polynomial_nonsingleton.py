...
out=Path('/tmp/audit_polynomial_nonsingleton');out.mkdir(exist_ok=True)
...
for mask in masks:
 ...
 for assignment in range(1<<len(bits)):
  ...
  if len(bits)<=4 or (len(bits)<=degree and (lambda ra,oa,rb,ob: __import__('math').comb(6-ra,3-oa)*__import__('math').comb(6-rb,3-ob) not in (0,1))(sum(bool(mask&(1<<i)) for i in range(0,12,2)),sum(bool(occ&(1<<i)) for i in range(0,12,2)),sum(bool(mask&(1<<i)) for i in range(1,12,2)),sum(bool(occ&(1<<i)) for i in range(1,12,2)))):
