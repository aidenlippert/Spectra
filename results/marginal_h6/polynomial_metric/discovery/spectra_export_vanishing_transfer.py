...
root=Path('/Users/aidenlippert/Documents/Spectra/results/marginal_h6/polynomial_metric');out=root/'vanishing_metric_transfer/proof';out.mkdir(exist_ok=True);proposal=json.loads((root/'vanishing_joint_metric_cone/proposal_-6.264.json').read_text());numerical=json.loads((root/'vanishing_metric_transfer/nonsingleton_quotient_dual/primal_proposal.json').read_text());proposal['numerator_bound']=next(v for l,v in zip(numerical['labels'],numerical['values']) if l==['b']);proposal['numerator_labels']=[l for l in numerical['labels'] if l!=['b']];proposal['numerator_values']=[v for l,v in zip(numerical['labels'],numerical['values']) if l!=['b']];c=json.loads((root/'vanishing_metric_transfer/candidate.json').read_text());metric_den=2*10**9;proof_den=10**12
metric_terms={}
