...
root=Path('/Users/aidenlippert/Documents/Spectra/results/marginal_h6/polynomial_metric');out=root/'sos_cutting_planes';out.mkdir(exist_ok=True)
...
basis=[sum(1<<i for i in inds) for degree in range(3) for inds in combinations(range(o.modes),degree)]
f=np.array([[(s&m)==m for m in basis] for s in states],dtype=float);history=[];squares=[]
for iteration in range(12):
 objective=np.zeros(a.shape[1]);objective[0]=-1;start=time.monotonic()
 r=linprog(objective,A_eq=a,b_eq=np.array([float(F(v,scale)) for v in p]),bounds=[(None,None)]+[(0,None)]*(a.shape[1]-1),method='highs-ipm',options={'time_limit':45})
 ...
 z=-r.eqlin.marginals;moment=f.T@(z[:,None]*f);eigen,vectors=np.linalg.eigh(moment);entry.update(bound=float(r.x[0]),minimum_moment_eigenvalue=float(eigen[0]),basis_degree=2)
 ...
 for index in np.flatnonzero(eigen<-1e-8)[:12]:
  coeff=np.round(vectors[:,index]*10**6).astype(np.int64);values=(f@coeff/10**6)**2;a=hstack([a,csc_matrix(values[:,None])],format='csc');labels.append(['sos',len(squares)]);squares.append({'denominator':10**6,'terms':[{'mask':m,'coefficient':int(v)} for m,v in zip(basis,coeff) if v]})
