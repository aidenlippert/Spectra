...
root=Path('/Users/aidenlippert/Documents/Spectra/results/marginal_h6/polynomial_metric');out=root/'joint_metric_cone';out.mkdir(exist_ok=True);data=json.loads((root/'candidate.json').read_text());o=JointPolynomial(data);states=spin_states(o.o.oracle);sa=np.array(states,dtype=np.int64);d=np.array([sum(((s>>(2*i))&3)==3 for i in range(o.sites))-1 for s in states]);q=np.array([[((s>>(2*i))&3).bit_count()-1 for i in range(o.sites)] for s in states]);ionic=d>=0;orbits=json.loads((root/'with_constant/proposal.json').read_text())['orbits'];features=np.array([sum(np.prod(q**np.array(powers),axis=1) for powers in orbit) for orbit in orbits]).T.astype(float);labels=[];ri=[];ci=[];values=[];vcols=[];start=time.monotonic()
