...
a=csc_matrix((np.asarray(values,dtype=float),(ri,ci)),shape=(len(rows),len(columns)));objective=np.zeros(len(columns));objective[0]=-1
print(json.dumps({'phase':'matrix','rows':len(rows),'columns':len(columns),'nnz':len(values),'seconds':time.monotonic()-start}),flush=True)
for name,scale in [('weight',compiled['receipt']['metric_scale']),('numerator',compiled['receipt']['numerator_scale'])]:
 poly={mask:int(value)/scale for mask,value in compiled[name]};start=time.monotonic();r=linprog(objective,A_eq=a,b_eq=[poly.get(m,0) for m in rows],bounds=bounds,method='highs',options={'time_limit':120})
 ...
 (root/(name+'_lp_receipt.json')).write_text(json.dumps(receipt,indent=2)+'\n');print(json.dumps(dict(phase=name,**receipt)),flush=True)
 if r.success:(root/(name+'_lp_solution.json')).write_text(json.dumps({'labels':[label for label,x in zip(labels,r.x) if abs(x)>1e-12],'values':[float(x) for x in r.x if abs(x)>1e-12]},indent=2)+'\n')
