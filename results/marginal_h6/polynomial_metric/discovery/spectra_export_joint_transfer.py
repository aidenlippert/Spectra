...
root=Path('/Users/aidenlippert/Documents/Spectra/results/marginal_h6/polynomial_metric');out=root/'joint_metric_transfer/proof';out.mkdir(exist_ok=True);proposal=json.loads((root/'joint_metric_cone/proposal_-6.24.json').read_text());numerical=json.loads((root/'joint_metric_transfer/nonsingleton_quotient_dual/primal_proposal.json').read_text());proposal['numerator_bound']=next(v for l,v in zip(numerical['labels'],numerical['values']) if l==['b']);proposal['numerator_labels']=[l for l in numerical['labels'] if l!=['b']];proposal['numerator_values']=[v for l,v in zip(numerical['labels'],numerical['values']) if l!=['b']];c=json.loads((root/'joint_metric_transfer/candidate.json').read_text());metric_den=10**9;proof_den=10**12
c['polynomial_metric']={'denominator':metric_den,'terms':[{'powers':powers,'coefficient':round(value*metric_den)} for orbit,value in zip(proposal['orbits'],proposal['metric_coefficients']) for powers in orbit if round(value*metric_den)]};c.update(kind='joint_polynomial_metric_gap_v1',target_lower=proposal['gamma']);ring=JointPolynomial(c);start=time.monotonic();weight,numerator,cost=ring.compile(F(c['target_lower']));construction={'compile_seconds':time.monotonic()-start,'compilation':cost,'discovery':'Joint metric/positivity LP on400 explicit spin-sector coordinates; exact number-ideal lift may have sector-sized cost.'}
