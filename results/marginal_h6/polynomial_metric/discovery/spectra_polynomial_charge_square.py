...
root=Path('/Users/aidenlippert/Documents/Spectra/results/marginal_h6/polynomial_metric');out=root/'charge_square_quotient';out.mkdir(exist_ok=True)
...
for degree in range(3):
 for bits in combinations(range(o.modes),degree):
  mask=sum(1<<i for i in bits)
  for assignment in range(1<<degree):
   occupied=sum(1<<i for j,i in enumerate(bits) if assignment&(1<<j));active=np.flatnonzero(((sa&mask)==occupied)&((d-1)!=0))
   if len(active):
    j=len(labels);labels.append(['square',mask,occupied]);ri.extend(active);ci.extend([j]*len(active));values.extend(((d[active]-1)**2).tolist())
