...
root=Path('/Users/aidenlippert/Documents/Spectra/results/marginal_h6/polynomial_metric/vanishing_metric_transfer');out=root/'nonsingleton_quotient_dual';out.mkdir(exist_ok=True)
...
receipt={'success':r.success,'status':r.message,'seconds':time.monotonic()-start,'rows':len(states),'columns':len(labels),'nnz':a.nnz,'scope':'Finite explicit400-state numerical quotient; b is FREE and objective maximizes b. All nonsingleton degree<=6 indicators, charge-localizers<=4; no ideal or zero columns.'}
if r.success:
