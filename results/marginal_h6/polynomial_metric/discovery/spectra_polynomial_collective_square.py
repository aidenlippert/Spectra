...
root=Path('/Users/aidenlippert/Documents/Spectra/results/marginal_h6/polynomial_metric');out=root/'collective_square_quotient';out.mkdir(exist_ok=True)
...
for sites in combinations(range(o.sites),3):
 for family,center in [('doublon',2),('number',5)]:
  ...
  if len(active):
   j=len(labels);labels.append(['collective_square',family,list(sites),center]);ri.extend(active);ci.extend([j]*len(active));values.extend((evaluated[active]**2).tolist())
