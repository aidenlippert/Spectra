...
root=Path('/Users/aidenlippert/Documents/Spectra/results/marginal_h6/polynomial_metric');out=root/'cubic_sos_simplex';out.mkdir(exist_ok=True)
...
for iteration in range(8):
 objective=np.zeros(a.shape[1]);objective[0]=-1;start=time.monotonic();r=linprog(objective,A_eq=a,b_eq=np.array([float(F(v,scale)) for v in p]),bounds=[(None,None)]+[(0,None)]*(a.shape[1]-1),method='highs-ds',options={'time_limit':60,'presolve':False})
