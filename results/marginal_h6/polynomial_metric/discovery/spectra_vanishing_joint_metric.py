...
root=Path('/Users/aidenlippert/Documents/Spectra/results/marginal_h6/polynomial_metric');out=root/'vanishing_joint_metric_cone';out.mkdir(exist_ok=True);data=json.loads((root/'candidate.json').read_text());o=JointPolynomial(data);states=spin_states(o.o.oracle);sa=np.array(states,dtype=np.int64);d=np.array([sum(((s>>(2*i))&3)==3 for i in range(o.sites))-1 for s in states]);q=np.array([[((s>>(2*i))&3).bit_count()-1 for i in range(o.sites)] for s in states]);ionic=d>=0;orbits=json.loads((root/'with_constant/proposal.json').read_text())['orbits'];features=np.array([sum(np.prod(q**np.array(powers),axis=1) for powers in orbit) for orbit in orbits]).T.astype(float);features=features*(d+1)[:,None];labels=[];ri=[];ci=[];values=[];vcols=[];start=time.monotonic()
...
a=csc_matrix((np.array(values,dtype=float),(ri,ci)),shape=(len(states),len(labels)));vcols=list(range(a.shape[1]));va=a;index={s:i for i,s in enumerate(states)};h=np.zeros((len(states),len(states)))
...
for gamma in [-6.24,-6.264]:
 ...
 r=linprog(cost,A_eq=matrix,b_eq=rhs,bounds=bounds,method='highs-ipm',options={'time_limit':90});receipt={'gamma':gamma,'success':r.success,'status':r.message,'seconds':time.monotonic()-start,'metric_features':features.shape[1],'scope':'Vanishing metric v=D*u with degree4 u; joint metric and nonsingleton-cone discovery in explicit400-state polynomial quotient; both metric and numerator receive their own positivity decompositions. Mean Q metric fixed1. No exact certificate yet.'}
 if r.success:
  ...
  proposal={'metric_form':'D_times_degree4','orbits':orbits,'metric_coefficients':w.tolist(),'metric_bound':.001,'numerator_bound':float(r.x[-1]),'weight_labels':[labels[j] for j in vcols],'weight_values':r.x[features.shape[1]:features.shape[1]+va.shape[1]].tolist(),'numerator_labels':labels,'numerator_values':r.x[features.shape[1]+va.shape[1]:-1].tolist(),'gamma':str(gamma)};(out/('proposal_'+str(gamma)+'.json')).write_text(json.dumps(proposal,indent=2)+'\n')
