...
root=Path('/Users/aidenlippert/Documents/Spectra/results/marginal_h6/polynomial_metric');out=root/'quadratic_sos_obstruction';out.mkdir(exist_ok=True)
c=json.loads((root/'candidate.json').read_text());o=JointPolynomial(c);compiled=json.loads((root/'compiled.json').read_text());proposal=json.loads((root/'full_sos/degree2_proposal.json').read_text());states=proposal['states'];rounded=[round(x*10**9) for x in proposal['dual_weights']];ionic=[any(((s>>(2*i))&3)==3 for i in range(o.sites)) for s in states];count=sum(ionic);rounded[ionic.index(True)]+=10**9-sum(rounded)
...
for mix in [10000,1000,100]:
 ...
 receipt.update(moment_matrix=psd,mixture_denominator=mix);c['functional']=functional;c['kind']='joint_polynomial_quadratic_sos_separator_v1';(out/'candidate.json').write_text(json.dumps(c,indent=2)+'\n');(out/'discovery_receipt.json').write_text(json.dumps(receipt,indent=2)+'\n');print(json.dumps(receipt),flush=True);break
