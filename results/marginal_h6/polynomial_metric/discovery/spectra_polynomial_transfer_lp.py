...
root=Path('/Users/aidenlippert/Documents/Spectra/results/marginal_h6/polynomial_metric/direct_transfer');data=json.loads((root/'candidate.json').read_text());o=JointPolynomial(data);compiled=json.loads((root/'compiled.json').read_text());modes=o.modes;degree=6
out=root/'reduced_lp';out.mkdir(exist_ok=True)
...
for mask in masks:
 ...
 for assignment in range(1<<len(bits)):
  ...
  if len(bits)<=4 or (occ==mask and all((mask&spin).bit_count()==o.target for spin in o.spin_masks)):
