...
cached={(x['creates'],x['annihilates']):x['upper'] for x in json.loads((root/'results/marginal_h6/charge_tail/dp_ratios.json').read_text())}
for (c,a),amp in o.groups.items():
 ...
 ratio=F(raw);ratios.append(float(ratio))
...
receipt={'success':r.success,'status':r.message,'bound':float(r.x[0]) if r.x is not None else None,'rows':len(rows),'columns':len(columns),'seconds':time.monotonic()-t,'minimum_doublons':2,'scope':'Numerical proposal only: grouped fixed-sign weighted row envelope using exact finite-memory metric-ratio maxima; degree4 Boolean positivity LP on D>=2.'}
(out/'dp_probe.json').write_text(json.dumps(receipt,indent=2)+'\n');print(json.dumps(receipt),flush=True)
if r.success:(out/'dp_lp_solution.json').write_text(json.dumps({'labels':labels,'values':r.x.tolist()},indent=2)+'\n')
