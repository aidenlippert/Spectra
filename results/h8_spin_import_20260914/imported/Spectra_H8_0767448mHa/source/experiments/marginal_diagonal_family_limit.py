...
def _replay(c, *, free_range_two_profile, full_quadratic_charge=False, charge_square_pairs=False, full_charge_indicators=False, full_signed_charge=False, hopping_telescope=False, spin_telescope=False, spectator_hopping=False, pair_transfer=False, two_spectator_hopping=False, three_spectator_hopping=False, coherent_projector=False, full_spin_word=False, pure_coherence=False, residual_coherence=False):
    ...
    if 'quadratic_charge_telescope' in c or 'charge_square_pair_telescope' in c or 'higher_charge_indicator_telescope' in c or 'signed_charge_telescope' in c or 'hopping_telescope' in c or 'spin_telescope' in c or 'spectator_hopping' in c or 'pair_transfer' in c or 'two_spectator_hopping' in c or 'three_spectator_hopping' in c or 'coherent_projector' in c or 'spin_word_telescope' in c or 'pure_coherence' in c or 'residual_coherence' in c:
    ...
    if type(pair_transfer) is not bool or (pair_transfer and not spectator_hopping):
        raise ValueError('Pair-transfer family requires spectator hopping mode')
    if type(two_spectator_hopping) is not bool or (two_spectator_hopping and not pair_transfer):
        raise ValueError('Two-spectator family requires pair-transfer mode')
    if type(three_spectator_hopping) is not bool or (three_spectator_hopping and not two_spectator_hopping):
        raise ValueError('Three-spectator hopping requires two-spectator hierarchy')
    if type(coherent_projector) is not bool or (coherent_projector and not three_spectator_hopping):
        raise ValueError('Coherent-projector family requires three-spectator hierarchy')
    if type(full_spin_word) is not bool or (full_spin_word and not coherent_projector):
        raise ValueError('Full spin-word family requires coherent-projector hierarchy')
    if type(residual_coherence) is not bool or (residual_coherence and not pure_coherence):
        raise ValueError('Residual coherence requires pure-coherence hierarchy')
    if type(pure_coherence) is not bool or (pure_coherence and not full_spin_word):
        raise ValueError('Pure coherence requires full spin-word hierarchy')
    if type(mixture) is not list or not 1 <= len(mixture) <= 9+len(shapes)+int(free_range_two_profile)+int(full_quadratic_charge)+4*int(charge_square_pairs)+6*int(full_charge_indicators)+36*int(full_signed_charge)+int(hopping_telescope)+4*int(spin_telescope)+14*int(spectator_hopping)+4*int(pair_transfer)+30*int(two_spectator_hopping)+18*int(three_spectator_hopping)+2*int(coherent_projector)+68*int(full_spin_word)+2*int(pure_coherence)+2*int(residual_coherence):
    ...
    spin_word_moments = {}
    if full_spin_word:
        from experiments.marginal_spin_word_telescope import PATTERNS, component
        spin_word_moments=dict.fromkeys(PATTERNS,F(0))
        for w,vector,norm in states:
            numerators=dict.fromkeys(PATTERNS,0)
            for s,a in vector.items():
                for five,orientation in ((s&1023,1),(s>>2,-1)):
                    entry=component(five)
                    if entry is not None:
                        label,sign=entry;numerators[label]+=orientation*sign*a*a
            for label,numerator in numerators.items():spin_word_moments[label]+=w*F(numerator,norm)
        if any(spin_word_moments.values()):raise ValueError('Every spin-word expectation must cancel exactly')
    hopping_moment = F(0)
    ...
    pair_moments = {}
    if pair_transfer:
        from experiments.marginal_pair_transfer import LABELS, actions
        for label in LABELS:
            action = actions({label:1})
            moment = sum((w*F(sum(a*b*vector.get(t,0) for s,a in vector.items() for t,b in action[s].items()),norm) for w,vector,norm in states),F(0))
            if moment:raise ValueError('Every pair-transfer expectation must cancel exactly')
            pair_moments[label] = str(moment)
    two_moments = {}
    if two_spectator_hopping:
        from experiments.marginal_two_spectator_hopping import LABELS, actions
        for label in LABELS:
            action = actions({label:1})
            moment = sum((w*F(sum(a*b*vector.get(t,0) for s,a in vector.items() for t,b in action[s].items()),norm) for w,vector,norm in states),F(0))
            if moment:raise ValueError('Every two-spectator hopping expectation must cancel exactly')
            two_moments[label] = str(moment)
    residual_moments = {}
    if residual_coherence:
        from experiments.marginal_residual_coherence import LABELS, actions
        for label in LABELS:
            action=actions({label:1})
            moment=sum((w*sum((a*b*vector.get(t,0) for s,a in vector.items() for t,b in action[s].items()),F(0))/norm for w,vector,norm in states),F(0))
            if moment:raise ValueError('Every residual-coherence expectation must cancel exactly')
            residual_moments[label]=str(moment)
    pure_moments = {}
    if pure_coherence:
        from experiments.marginal_pure_coherence import LABELS, actions
        for label in LABELS:
            action=actions({label:1})
            moment=sum((w*sum((a*b*vector.get(t,0) for s,a in vector.items() for t,b in action[s].items()),F(0))/norm for w,vector,norm in states),F(0))
            if moment:raise ValueError('Every pure-coherence expectation must cancel exactly')
            pure_moments[label]=str(moment)
    coherent_moments = {}
    if coherent_projector:
        from experiments.marginal_coherent_projector_telescope import LABELS, actions
        for label in LABELS:
            action=actions({label:1})
            moment=sum((w*sum((a*b*vector.get(t,0) for s,a in vector.items() for t,b in action[s].items()),F(0))/norm for w,vector,norm in states),F(0))
            if moment:raise ValueError('Every coherent-projector expectation must cancel exactly')
            coherent_moments[label]=str(moment)
    three_moments = {}
    if three_spectator_hopping:
        from experiments.marginal_three_spectator_hopping import LABELS, actions
        for label in LABELS:
            action = actions({label:1})
            moment = sum((w*F(sum(a*b*vector.get(t,0) for s,a in vector.items() for t,b in action[s].items()),norm) for w,vector,norm in states),F(0))
            if moment:raise ValueError('Every three-spectator hopping expectation must cancel exactly')
            three_moments[label] = str(moment)
    ...
    if pair_transfer:
        result['pair_transfer_moments'] = pair_moments
    if two_spectator_hopping:
        result['two_spectator_hopping_moments'] = two_moments
    if residual_coherence:
        result['residual_coherence_moments']=residual_moments
    if pure_coherence:
        result['pure_coherence_moments']=pure_moments
    if full_spin_word:
        result['spin_word_moments']={key:str(value) for key,value in spin_word_moments.items()}
    if coherent_projector:
        result['coherent_projector_moments']=coherent_moments
    if three_spectator_hopping:
        result['three_spectator_hopping_moments'] = three_moments
    return result
