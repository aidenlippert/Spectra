...
def append_groups(groups,additional,m,signature):
    """Validate optional local polynomial blocks before any coefficient maps."""
    result=list(groups)
    for group in additional or []:
        polys=group['polynomials']
        if not polys:raise ValueError('Empty additional polynomial block')
        keys=set()
        for p in polys:
            if not p or canonical(p)!=p:raise ValueError('Noncanonical or empty additional polynomial')
            for w,c in p.items():
                if len(w)>3 or not isinstance(c,(int,F)) or isinstance(c,bool):raise ValueError('Exact degree-three polynomials required')
                if any(type(flag) is not int or flag not in (0,1) or type(i) is not int or not 0<=i<m for flag,i in w):raise ValueError('Invalid additional orbital')
                keys.add((sum(2*flag-1 for flag,_ in w),signature(w)))
        if len(keys)!=1:raise ValueError('Additional block mixes charge or symmetry')
        if 'merge_into' in group:
            matches=[i for i,g in enumerate(result) if g['name']==group['merge_into']]
            if len(matches)!=1:raise ValueError('Unique merge target required')
            index=matches[0];target=result[index]
            target_keys={(sum(2*flag-1 for flag,_ in w),signature(w)) for p in target['polynomials'] for w in p}
            if target_keys!=keys:raise ValueError('Merge target has different charge or symmetry')
            result[index]={**target,'polynomials':target['polynomials']+list(polys)}
        else:
            result.append({'name':str(group['name']),'polynomials':polys})
    return result


def run(h,m,n,out,solver_seconds=60,enrich=True,creator_channels=False,one_body_steps=0,diagonal_driver=False,solver='CLARABEL',map_backend='exact',row_condition=False,basis_condition='none',additional_groups=None):
    ...
    groups=append_groups(groups,additional_groups,m,signature)
    additional_count=len(additional_groups or [])
    if additional_count:
        (out/'additional_groups.json').write_text(json.dumps([{**{k:v for k,v in g.items() if k!='polynomials'},'polynomials':[encode(p) for p in g['polynomials']]} for g in additional_groups],separators=(',',':'))+'\n')
    ...
    (out/'pre_solve.json').write_text(json.dumps({'stage':'solver_starting','construction_seconds':built-start,
        'modes':m,'particles':n,'one_body_steps':one_body_steps,'diagonal_driver':diagonal_driver,'additional_groups':additional_count,
    ...
    if problem.constraints[0].dual_value is not None:
        (out/'dual_proposal.json').write_text(json.dumps({'modes':m,'particles':n,
            'hamiltonian':encode(h),'enrich':enrich,'creator_channels':creator_channels,'one_body_steps':one_body_steps,'diagonal_driver':diagonal_driver,'additional_groups':additional_count,
    ...
    receipt={'method':'H_only_cubic_commutator_plus_quadratic' if enrich else 'quadratic_baseline_body2_ideal',
             ...
             'basis_condition':basis_condition,'basis_conditioning':conditioning,'creator_channels':creator_channels,'one_body_steps':one_body_steps,'diagonal_driver':diagonal_driver,'additional_groups':additional_count,'source_factors_used':False,'source_upper_used':False,'scope':'Explicit polynomial span; caller must account for additional-group selection. No omitted-cone optimality or generic accuracy guarantee.'}
    (out/'receipt.json').write_text(json.dumps(receipt,indent=2)+'\n');print(json.dumps({k:v for k,v in receipt.items() if k not in ('polynomial_term_counts','polynomial_Gram_dimensions','map_stats','basis_conditioning')}),flush=True);return receipt
