"""Production integration of the two exact pure-coherence directions."""
...
import subprocess
import pytest
from experiments.marginal_pure_coherence import LABELS, build, coefficients, actions
...
from test_marginal_spin_word_telescope import toy as word_toy
...
def toy():
    c = word_toy()
    c['kind'] = 'hubbard_projector_extension_v19'
    c['pure_coherence'] = {key: '1/1000' for key in LABELS}
    return c
...
def test_matches_independently_accepted_sparse_operator(case, label):
    source = json.loads((ROOT / f'results/marginal_graded_hubbard8/spin_coherence/{case}/telescope_replay.json').read_text())
...
def test_all_older_versions_refuse_new_field(version):
    ...
    with pytest.raises(ValueError, match='requires v19'):
        replay(c)
...
def test_exact_family_moments_and_fixed_field_refusal():
    c = family()
    c['kind'] = 'joint_diagonal_range2_family_limit_v15'
    r = family_replay(c)
    assert r['pure_coherence_moments'] == dict.fromkeys(LABELS, '0')
    assert len(r['spin_word_moments']) == 120 and r['periodic_family_upper'] == '13/5'
    c['pure_coherence'] = {next(iter(LABELS)): '1/1000'}
    with pytest.raises(ValueError, match='constrain moments'):
        family_replay(c)


@pytest.mark.parametrize('case,selection', [('W_zero', 'scaled'), ('W_plus_1', 'final')])
def test_previous_nonstationary_mixture_cannot_be_relabelled(case, selection):
    c = json.loads((ROOT / f'results/marginal_graded_hubbard8/spin_word/{case}/{selection}/range_two_family_limit_certificate.json').read_text())
    c['kind'] = 'joint_diagonal_range2_family_limit_v15'
    with pytest.raises(ValueError, match='pure-coherence expectation'):
        family_replay(c)
...
def test_preserved_old_source_cap_and_new_hierarchy():
    ...
    f.update(pair_transfer=True, two_spectator_hopping=True, three_spectator_hopping=True,
             coherent_projector=True, full_spin_word=True)
    c['mixture'] = [{'weight': '1/201', 'vector': {0: 1}}]*201
    with pytest.raises(ValueError, match='constraint count'):
        _replay(c, **f)
    assert _replay(c, **f, pure_coherence=True)['mixture_sources'] == 201
    c['mixture'] = [{'weight': '1/202', 'vector': {0: 1}}]*202
    with pytest.raises(ValueError, match='constraint count'):
        _replay(c, **f, pure_coherence=True)
    f['full_spin_word'] = False
    with pytest.raises(ValueError, match='requires full spin-word'):
        _replay(c, **f, pure_coherence=True)
    with pytest.raises(ValueError, match='requires full spin-word'):
        _replay(c, **f, pure_coherence=1)


def test_old_family_driver_refuses_new_energy(tmp_path):
    source = ROOT / 'results/marginal_graded_hubbard8/spin_word/W_zero/scaled'
    ...
    assert result.returncode != 0 and 'Pure-coherence energy requires a matching enlarged family cap' in result.stderr
