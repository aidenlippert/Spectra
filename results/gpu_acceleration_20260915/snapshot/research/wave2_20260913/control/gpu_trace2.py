...
def run(fixture_path, basis_path, cpu_path, out_path):
    ...
    def assemble():
        ...
        M.sum_duplicates(); print("AFTER_SUM",M.diagonal()[:3].get().tolist(),"ZEROCOUNT",int(cp.count_nonzero(M.data==0).get()),flush=True)
        M.data /= D
        print("AFTER_DIVIDE",M.diagonal()[:3].get().tolist(),flush=True)
