...
from experiments.marginal_symbolic import decode,canonical,mono
...
def embed_paired_seed(old_words,V,new_words,pair):
    """Include the old paired span before adding independent responses."""
    size=pair['joint_size'];nc=pair['cubic_size'];nl=size-nc
    loc={}
    for i,w in enumerate(new_words):
        p=canonical(mono(w))
        if len(p)!=1:raise ValueError('Seed dictionaries must be monomials')
        cw,s=next(iter(p.items()));loc[cw]=(i,s)
    J=np.zeros((size,V.shape[1]+(nl if nc else 0)))
    for i,w in enumerate(old_words):
        p=canonical(mono(w))
        if len(p)!=1:raise ValueError('Seed dictionaries must be monomials')
        cw,s=next(iter(p.items()))
        if cw not in loc:raise ValueError('Old paired word absent from response dictionary')
        j,t=loc[cw];slot=pair['slot_indices'][0][j]
        J[slot,:V.shape[1]]+=float(s/t)*V[i]
        if nc and j>=nc:
            J[pair['slot_indices'][1][j],:V.shape[1]]+=float(s/t)*V[i]
    if nc and nl:J[nc:,V.shape[1]:]=np.eye(nl)
    return J
