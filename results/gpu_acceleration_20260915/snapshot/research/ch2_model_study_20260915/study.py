...
def evaluate(basis, spin, x, log):
    ...
    mf.irrep_nelec = {'A1': (3, 2), 'B1': (1, 0), 'B2': (1, 1)} if spin else {'A1': 6, 'B2': 2}
    mf.conv_tol = 1e-11
    mf.kernel()
    if not mf.converged:
    ...
    mc.fcisolver.wfnsym = 'B1' if spin else 'A1'
    mc.kernel(mo)
    ...
    row = {'bond_Angstrom': float(x[0]), 'angle_degrees': math.degrees(x[1]),
           'CASSCF_energy_Ha': float(mc.e_tot), 'measured_S2': float(ss),
...
def calculate(basis, spin):
    ...
    dump(directory/'result.json', {'basis': basis, 'total_spin': spin, 'state_symmetry': 'B1' if spin else 'A1',
        'physical_electrons': 8, 'active_electrons': 6, 'active_spatial_orbitals': 6, 'frozen_core_orbitals': 1,
