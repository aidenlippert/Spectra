...
def run():
    ...
    for T in (F(1,5),F(1,2)):
     ...
     for repeat in range(7):
      ...
      for cap in arms:
       ...
       try:
        p,w,c=adaptive_with_square(g,o,T,TOL,term_cap=cap,trigger_ratio=F(11,10),block_groups=4)
 ...
 result=dict(rows=rows,protocol_sha256=sha256((ROOT/'research/v7/BLOCK_PROTOCOL.md').read_bytes()).hexdigest(),checker_sha256=sha256((ROOT/'experiments/v7_quadratic_certificate.py').read_bytes()).hexdigest(),autonomous_acquisition='not_run')
 (ROOT/'results/v7/block_cost.json').write_text(json.dumps(result,indent=2));return result
