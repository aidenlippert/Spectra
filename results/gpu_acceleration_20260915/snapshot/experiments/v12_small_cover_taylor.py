"""V11 integer recurrence with a bounded small-cover construction hook.

Historical V8/V11 sources and both trusted checkers remain frozen.
"""
from fractions import Fraction as F
from math import gcd, factorial
from .v7_certificate import Piece, norm_witness, clean, rational, BudgetExceeded
from .pauli import commutator_i
from .v11_guarded_taylor import norm_rejection
from .v12_small_cover import small_cover,evolution_witness
...
def small_cover_taylor(gen, initial, duration, tolerance, max_order=24, guard="frobenius", enabled=True):
    ...
    for k in range(max_order + 1):
        ...
        for grouping in ('l1', 'firstfit', 'weighted'):
            ...
            if bound <= tolerance:
                ...
                return Piece(duration, tuple(coeffs)), witness, dict(probes=probes, denominator=d, initial_denominator=q0, integer_work=integer_cost, generator=dict(gen.cost), guard_work=gate_cost, cover_calls=cover_calls)
        if enabled and len(derivative)<=40 and min(ordinary_bounds)<=F(11,10)*tolerance:
            cover, work = small_cover(derivative,{mode:partitions[mode] for mode in ('weighted','firstfit')})
            covered_bound=duration**(k+1)/F(k+1)*F(cover['claimed_bound'])
            cover_calls.append(dict(order=k,bound=str(covered_bound),work=work))
            if covered_bound<=tolerance:
                witness=evolution_witness(cover,k,bool(derivative),covered_bound)
                return Piece(duration,tuple(coeffs)),witness,dict(probes=probes,denominator=d,initial_denominator=q0,
                    integer_work=integer_cost,generator=dict(gen.cost),guard_work=gate_cost,cover_calls=cover_calls)
        if k < max_order: b = b_next
