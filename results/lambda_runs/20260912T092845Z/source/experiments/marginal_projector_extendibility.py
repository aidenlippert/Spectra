...
def replay(certificate):
    if type(certificate) is dict and certificate.get('kind') in ('hubbard_projector_extension_v2','hubbard_projector_extension_v4'):
...
def _replay_extended(c):
    ...
    joint = c.get('kind') == 'hubbard_projector_extension_v4'
    ...
    for key,(_,matrix,columns) in data.items():
        ...
        base=[[x-(lower*norms[i] if i==j else 0) for j,x in enumerate(row)] for i,row in enumerate(matrix)]
