...
def expand_squares(blocks, denominator, modes, max_degree=3):
    ...
    for block in blocks:
        ...
        for i, wi in enumerate(words):
            left = tuple((1-c, m) for c, m in reversed(wi))
            for j, wj in enumerate(words):
                coefficient = sum(row[i]*row[j] for row in factors)
                if not coefficient:
                    continue
                for word, sign in word_product(left, wj):
...
def verify(certificate):
    ...
    return {"lower": str(b-eta), "lower_float": float(b-eta),
