...
def arm(name, out, budget, started, adaptive_source=None):
    from research.spin_subspace_20260913.discovery import Model
    ...
    def trial(label, span, separate=False, solver_cap=30.):
    ...
    if name == 'adaptive':
        ...
        for rnd in range(1, 5):
            ...
            for per_group in (1, 2):
                ...
                if not extra or key in seen or len(proposed) > 32:
                    continue
                seen.add(key)
                if remaining() < 30:
        ...
        trial('full_frame', model.full_span(), solver_cap=60.)
    elif name == 'separate':
        ...
        summary['descriptor_source'] = str(source_path.relative_to(ROOT))
        summary['descriptor_discovery_cost_seconds'] = source['elapsed_seconds']
        trial('separate_selected', span, separate=True, solver_cap=60.)
    else:
...
def run(out):
    ...
    for name, budget in [('adaptive', 180.), ('separate', 90.), ('full', 90.)]:
        before = time.monotonic()
        cmd = [sys.executable, '-m', 'research.spin_subspace_20260913.campaign', '--arm', name,
            '--out', str(out/name), '--budget', str(budget), '--started', str(before)]
        if name == 'separate':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--arm', choices=['adaptive', 'separate', 'full']); parser.add_argument('--budget', type=float, default=180.)
    parser.add_argument('--started', type=float); parser.add_argument('--adaptive-source', type=Path)
    args = parser.parse_args()
    if args.arm:
        arm(args.arm, args.out, args.budget, args.started, args.adaptive_source)
    else:
        run(args.out)
