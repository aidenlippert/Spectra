...
def build(case):
    ...
    groups, specs = clustered_frame(m, design['clusters'], design['collective_pairs'], design.get('complete', False))
    number_basis = ideal_basis(m, design['clusters'], design.get('complete', False))
    ...
    ladder_basis = [mono(((1, i), (0, j))) for i in range(1, m, 2) for j in range(0, m, 2)]
    ...
    rec = {'kind': 'direct_local_collective_preparation_v1', 'fixture': str(case/'fixture.json'),
        ...
        'moment_transfer_steps': oracle.steps, 'moment_pullback_residual': float(pullback[3]),
