...
from .collective_control import controls,comm,trig_interval,pi_interval
...
def calculate(data,cert,control_receipt,amplitude=F(1,2)):
    ...
    theta=F(control_receipt['angle']);pilo,pihi=pi_interval()
    if not pihi/2<theta<pilo:raise ValueError('Fixed-angle obstruction formula domain')
    if any(F(x)<0 for x in control_receipt['uncertainties'].values()):raise ValueError('Negative uncertainty')
    ...
    cl,ch=trig_interval(theta);sl,sh=trig_interval(theta,True)
