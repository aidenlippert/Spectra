...
from experiments.marginal_signed_charge_telescope import PATTERNS


def solve_basis(matrix,rhs):
    m=len(rhs);n=len(matrix[0])
    if not 1<=n<=m<=66 or any(len(row)!=n for row in matrix):raise ValueError('Bounded rectangular dual basis required')
...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('directory',type=Path);parser.add_argument('--scale',type=float,default=.02);args=parser.parse_args();out=args.directory
    ...
    for key,a,ds,ph,q,ts,cols,k,derivatives in mats:
        m=a+float(alpha)*ph+float(beta)*q+np.diag(np.array(taus)@ts)
        ...
        directions=[np.zeros(63)]+[sign*np.eye(63)[j]*args.scale for j in range(63) for sign in (-1,1)]+[rng.normal(size=63)*args.scale for _ in range(16)]
        for delta in directions:
            _,vectors=eigh(m+np.einsum('i,ijk->jk',delta[:2],ds)+np.diag(delta[2:]@ts),subset_by_index=[0,0]);v=vectors[:,0]/scale;coeff=[round(float(z/max(abs(v)))*10**8) for z in v]
            vector={str(s):z*b for z,col in zip(coeff,cols) if z for s,b in col.items()};norm=sum(v*v for v in vector.values());w=np.array(coeff,float)*scale/np.sqrt(float(norm));d=np.array([w@matrix@w for matrix in ds]);row=np.r_[1,d,ts@(w*w),w@ph@w,w@q@w]
    ...
    arr=np.array(rows).T;rhs=[F(1)]+[F(0)]*63+[th,tj]
    result=linprog(energies,A_eq=arr[:64],b_eq=np.array(rhs[:64],float),A_ub=arr[64:],b_ub=np.array(rhs[64:],float),bounds=(0,None),method='highs',options={'dual_feasibility_tolerance':1e-9,'primal_feasibility_tolerance':1e-9})
    diagnostic={'accepted':False,'scale':args.scale,'states':len(states),'active_sectors':list(exact),'status':result.message,'seconds':time.monotonic()-started}
    if result.success:
        values=list(result.x)+list(np.array(rhs[64:],float)-arr[64:]@result.x);support=[i for i,v in enumerate(values) if v>1e-10];diagnostic.update(numerical_family_upper=float(result.fun)/5,basis_size=len(support))
        erows=[];eenergies=[]
        for index in support:
            if index>=len(states):erows.append([F(int(j==64+index-len(states))) for j in range(66)]);eenergies.append(F(0));continue
            vector,norm,coeff,key=states[index];k,derivatives,cols,ts=exact[key]
