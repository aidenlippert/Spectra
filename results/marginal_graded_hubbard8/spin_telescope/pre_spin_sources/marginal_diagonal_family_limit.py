...
def _replay(c, *, free_range_two_profile, full_quadratic_charge=False, charge_square_pairs=False, full_charge_indicators=False, full_signed_charge=False, hopping_telescope=False):
    ...
    if 'quadratic_charge_telescope' in c or 'charge_square_pair_telescope' in c or 'higher_charge_indicator_telescope' in c or 'signed_charge_telescope' in c or 'hopping_telescope' in c:
    ...
    if type(mixture) is not list or not 1 <= len(mixture) <= 9+len(shapes)+int(free_range_two_profile)+int(full_quadratic_charge)+4*int(charge_square_pairs)+6*int(full_charge_indicators)+36*int(full_signed_charge)+int(hopping_telescope):
