...
def _replay(c, *, free_range_two_profile, full_quadratic_charge=False, charge_square_pairs=False, full_charge_indicators=False, full_signed_charge=False, hopping_telescope=False, spin_telescope=False, spectator_hopping=False, pair_transfer=False):
    ...
    if type(mixture) is not list or not 1 <= len(mixture) <= 9+len(shapes)+int(free_range_two_profile)+int(full_quadratic_charge)+4*int(charge_square_pairs)+6*int(full_charge_indicators)+36*int(full_signed_charge)+int(hopping_telescope)+4*int(spin_telescope)+14*int(spectator_hopping)+4*int(pair_transfer):
