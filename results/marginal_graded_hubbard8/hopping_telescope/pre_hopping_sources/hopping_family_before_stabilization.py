...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('directory',type=Path);parser.add_argument('--scale',type=float,default=.002);parser.add_argument('--determinant-anchors',action='store_true');parser.add_argument('--pricing-rounds',type=int,default=0);args=parser.parse_args();out=args.directory
    ...
    for iteration in range(args.pricing_rounds):
        ...
        arr=np.array(rows).T
        result=linprog(energies,A_eq=arr[:65],b_eq=np.array(rhs[:65],float),A_ub=arr[65:],b_ub=np.array(rhs[65:],float),bounds=(0,None),method='highs',options={'dual_feasibility_tolerance':1e-9,'primal_feasibility_tolerance':1e-9})
    diagnostic={'accepted':False,'scale':args.scale,'pricing':pricing,'determinant_anchors':args.determinant_anchors,'states':len(states),'active_sectors':list(exact),'status':result.message,'seconds':time.monotonic()-started}
    if result.success:
