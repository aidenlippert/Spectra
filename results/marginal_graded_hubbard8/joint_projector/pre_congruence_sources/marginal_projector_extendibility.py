...
def _psd(matrix):
    ...
    return integer_psd([[int(x*scale) for x in row] for row in matrix])


def _rank_one_psd(base,weights,penalty,norm):
    ...
    return integer_psd(matrix,initial_divisor=s)
...
def projector_bound(vector, windows, ceiling,block_sites=4):
    ...
    for sector,gram in grams.items():
        ...
        receipts.append({'sector':list(sector),'psd':_psd(matrix)})
...
def replay(certificate):
    if type(certificate) is dict and certificate.get('kind') in ('hubbard_projector_extension_v2','hubbard_projector_extension_v4','hubbard_projector_extension_v5'):
        return _replay_extended(certificate)
...
def _replay_extended(c):
    ...
    if type(local) is not dict or local.get('kind')!='local_hubbard_block_v1' or type(target) is not dict:
    ...
    joint = c.get('kind') in ('hubbard_projector_extension_v4','hubbard_projector_extension_v5')
    telescope={}
    if c.get('kind')=='hubbard_projector_extension_v5':
    ...
    if joint:
        ...
        joint_overlap=joint_projector_bound(c.get('vector'),spec.get('vector'),spec.get('windows'),
                                            ratio,spec.get('projector_sum_ceiling'))
        if sites<=joint_overlap['support_sites']:
            raise ValueError('Chain must exceed joint projector inequality support')
    overlap=projector_bound(c.get('vector'),c.get('windows'),c.get('projector_sum_ceiling'),L)
    ...
    for key,(_,matrix,columns) in data.items():
        ...
        corrections=[]
        for col in columns:
            values={telescope.get(s&1023,F(0))-telescope.get(s>>2,F(0)) for s in col} if telescope else {F(0)}
        ...
        receipts.append({'sector':list(key),'psd':_rank_one_psd(base,weights,coefficient,source_norm)})
    theta=F(overlap['average_fidelity_ceiling'])
    joint_cost=beta*F(joint_overlap['average_ceiling']) if joint else F(0)
    periodic=(lower-penalty*theta-joint_cost)/(L-1);opened=periodic-(2*t+abs(V))/sites
