...
def build(x,V=F(1,2),W=None,range_profile=None,quadratic_terms=None,square_terms=None):
    u,t,v=profiles(x,V)
    ...
    for key,(states,matrix,cols) in data.items():
        diagonal=[]
        for col in cols:
            values={diagonal_value(s,density2)+(quadratic_value(s,quadratic_terms) if quadratic_terms else 0)+(square_value(s,square_terms) if square_terms else 0) for s in col}
...
def main():
    ...
    diagonal_count=len(Ys)+int(args.free_range_two_profile)+int(args.quadratic_charge)+square_count
    ...
    for row in cases:
        ...
        fresh=build(rational[:6],args.target_v,args.target_w,range_profile,quadratic_terms,square_terms);minimum=(float('inf'),None);max_difference=0.
        for key,A,deriv,P,Q,T in mats:
            ...
            if square_count:affine=affine+np.diag(np.array(list(square_terms.values()),float)@T[-square_count:])
