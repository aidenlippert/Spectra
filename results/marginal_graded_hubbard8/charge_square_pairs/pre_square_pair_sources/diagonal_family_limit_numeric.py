...
def main():
    global OUT
    parser=argparse.ArgumentParser();parser.add_argument('--directory',type=Path,default=OUT);parser.add_argument('--range-two',action='store_true');parser.add_argument('--free-range-two-profile',action='store_true');parser.add_argument('--quadratic-charge',action='store_true')
    ...
    diagonal_count=9+int(args.free_range_two_profile)+int(args.quadratic_charge)
    ...
    original = build(x,W=W,range_profile=range_profile,quadratic_terms=quadratic_terms)
    perturbed = []
    for j in range(6):
        point = x.copy()
        point[j] += F(1, 100)
        perturbed.append(build(point,W=W,range_profile=range_profile,quadratic_terms=quadratic_terms))
