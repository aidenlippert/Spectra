...
def _replay(c, *, free_range_two_profile, full_quadratic_charge=False):
    ...
    if 'quadratic_charge_telescope' in c:
    ...
    if type(mixture) is not list or not 1 <= len(mixture) <= 9+len(shapes)+int(free_range_two_profile)+int(full_quadratic_charge):
