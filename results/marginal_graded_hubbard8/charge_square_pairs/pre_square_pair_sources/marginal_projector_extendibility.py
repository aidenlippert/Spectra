...
def replay(certificate,*,psd_witnesses=None):
    if type(certificate) is dict and certificate.get('kind') in ('hubbard_projector_extension_v2','hubbard_projector_extension_v4','hubbard_projector_extension_v5','hubbard_projector_extension_v6','hubbard_projector_extension_v7'):
...
def _replay_extended(c,proof=None):
    ...
    range_two=c.get('kind') in ('hubbard_projector_extension_v6','hubbard_projector_extension_v7')
    quadratic=c.get('kind')=='hubbard_projector_extension_v7'
    ...
    joint = c.get('kind') in ('hubbard_projector_extension_v4','hubbard_projector_extension_v5','hubbard_projector_extension_v6','hubbard_projector_extension_v7')
    telescope={}
    if c.get('kind') in ('hubbard_projector_extension_v5','hubbard_projector_extension_v6','hubbard_projector_extension_v7'):
    ...
    for key,(_,matrix,columns) in data.items():
        ...
        corrections=[]
        for col in columns:
            values={telescope.get(s&1023,F(0))-telescope.get(s>>2,F(0)) + (diagonal_value(s,range_profile) if range_two else 0) + (local_value(s,quadratic_terms) if quadratic else 0) for s in col}
