...
def solve_basis(matrix,rhs):
    ...
    if not 1<=n<=m<=137 or any(len(row)!=n for row in matrix):raise ValueError('Bounded rectangular dual basis required')
...
def main():
    ...
    if args.pricing_radius:raise ValueError('Clipped dual pricing disabled; candidate-feasible box used')
    ...
    for key,a,ds,ph,q,ts,cols,k,derivatives in mats:
        ...
        directions=[np.zeros(134)]+[sign*np.eye(134)[j]*args.scale for j in range(134) for sign in (-1,1)]+[rng.normal(size=134)*args.scale for _ in range(16)]
        for delta in directions:
            _,vectors=eigh(m+np.einsum('i,ijk->jk',delta[:2],ds)+np.diag(delta[2:63]@ts)+np.einsum('i,ijk->jk',delta[63:134],h),subset_by_index=[0,0]);v=vectors[:,0]/scale;coeff=[round(float(z/max(abs(v)))*10**8) for z in v]
    ...
    if len(states)>2000:raise ValueError('State-sampling budget exceeded')
    ...
    if len(states)>6096:raise ValueError('Total physical candidate budget exceeded')
    ...
    for iteration in range(args.pricing_rounds):
        ...
        bounds=[(float(z-.05),float(z+.05)) for z in center]
        ...
        pricing.append({'round':iteration,'sampled_upper':float(result.fun)/5,'minimum_reduced_eigenvalue':lowest,'added':added,'trust_dual_objective':float(np.array(rhs,float)@dual)/5,'candidate_dual_violation':candidate_violation})
        print(json.dumps(pricing[-1]),flush=True)
        if not added:break
        if len(states)>13000:raise ValueError('Pricing physical candidate cap exceeded')
        arr=np.array(rows).T
        result=linprog(energies,A_eq=arr[:135],b_eq=np.array(rhs[:135],float),A_ub=arr[135:],b_ub=np.array(rhs[135:],float),bounds=(0,None),method='highs',options={'dual_feasibility_tolerance':1e-10,'primal_feasibility_tolerance':1e-10})
    diagnostic={'source_sha256':{str(p.resolve().relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path(__file__),args.extra_atoms,out/'profile_joint_r1_2_certificate.json']},'accepted':False,'proposal_written':False,'lp_tolerance':1e-10,'support_threshold':1e-14,'scale':args.scale,'extra_atom_file':str(args.extra_atoms),'extra_sampled_vectors':added_atoms,'pricing':pricing,'pricing_eigenvectors':args.pricing_eigenvectors,'pricing_radius':args.pricing_radius,'determinant_anchors':args.determinant_anchors,'states':len(states),'active_sectors':list(exact),'status':result.message,'seconds':time.monotonic()-started}
