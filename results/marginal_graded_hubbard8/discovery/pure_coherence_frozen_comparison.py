...
def main():
    ...
    expected=copy.deepcopy(new);expected.pop('pure_coherence');expected['kind']='hubbard_projector_extension_v18';expected['penalized_lower']=ablation['penalized_lower']
    if expected!=ablation:raise ValueError('Ablation changed other coefficients')
    previous_source=BASE.parent/'spin_word/held_out/profile_joint_r1_2_certificate.json'
    ...
    result={'accepted':True,'target':target,'source_recipe':'Initial W0 pure-coherence certificate, before polish.',
            'with_pure_coherence_open_lower':str(lower),'without_pure_coherence_open_lower':no_new['lower_per_site'],
            ...
            'scope':'Exact frozen-source/penalty/correction transfer to U5,t1,V1/4,W-1/5 on the same geometry and filling. Two-term pure-coherence fixed-recipe ablation and previous frozen-recipe comparison use accepted energy proofs. Neither comparison caps a reoptimized older family. No generic molecular or scalability conclusion.'}
