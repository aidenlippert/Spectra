...
def main():
    ...
    basis={tuple(item['sector']):np.array(item['basis']) for item in json.loads((args.seed.parent/'subspace_basis.json').read_text())};history=[];center=y0.copy();radius=.05
    ...
    def enrich(y):
        ...
        for e,key,v in spectra:
            if e>max(minimum+.01,y[-1]+1e-7) and key not in basis:continue
            ...
            basis[key]=u[:,:n]
        if len(basis)>32:raise RuntimeError('active block cap32 reached')
    evaluate(center)
    outcome=SimpleNamespace(success=False,message='bounded subspace round limit')
    for iteration in range(1):
        try:assert max(v.shape[1] for v in basis.values())<=48
        except RuntimeError as error:
            outcome.message=str(error);break
        dual_constraints={}
        var=cp.Variable(85)
        constraints=[var[:2]>=0,var[:2]<=2.5,cp.sum(var[:2])<=2.5,var[2:4]>=0,var[2:4]<=3,var[4:84]>=-2,var[4:84]<=2,var[-1]>=-10,var[-1]<=0,var[:84]>=y0[:84]-radius,var[:84]<=y0[:84]+radius]
        for item in mats:
            key,a,ds,ph,q,ts,*_=item
            ...
            cone=fixed+cp.reshape(coefficient@var,(n,n),order='C')>>0
            constraints.append(cone);dual_constraints[key]=cone
        ...
        entry={'round':iteration,'status':problem.status,'active_blocks':len(basis),'largest_compressed_dimension':max(v.shape[1] for v in basis.values()),'solver_seconds':problem.solver_stats.solve_time}
        if var.value is None:
            history.append(entry);outcome.message='conic solver returned no candidate';break
        atoms=[]
        for key,cone in dual_constraints.items():
            ev,vec=np.linalg.eigh(cone.dual_value)
            for i,value in enumerate(ev):
                if value>1e-9:atoms.append({'sector':key,'dual_weight':float(value),'coordinates':(basis[key]@vec[:,i]).tolist()})
        (args.output/'dual_atoms.json').write_text(json.dumps({'accepted':False,'atoms':atoms,'trust_boundary_distance':float(np.min(radius-abs(var.value[:84]-y0[:84]))),'solver_status':problem.status})+'\n')
        center=var.value
