...
def main():
    ...
    outcome=None
    try:
        outcome=minimize(lambda y:float(cost@y),y0,jac=lambda y:cost,method='SLSQP',bounds=[(0,2.5)]*2+[(0,3)]*2+[(-2,2)]*80+[(-10,0)],constraints=[{'type':'ineq','fun':lambda y:evaluate(y)[0],'jac':lambda y:evaluate(y)[1]},{'type':'ineq','fun':lambda y:2.5-y[0]-y[1],'jac':lambda y:np.r_[-1.,-1.,np.zeros(83)]}],options={'maxiter':240,'ftol':1e-10})
    ...
    (args.output/'spectator_proposal.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
