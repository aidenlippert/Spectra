...
def reconstruct(mixture):
    if not 1<=len(mixture)<=139:raise ValueError('At most139 physical mixture sources required')
    sources=[]
    for item in mixture:
...
def main():
    ...
    if not r.get('accepted') or not r.get('coherent_projector'):raise ValueError('Accepted coherent-projector family required')
