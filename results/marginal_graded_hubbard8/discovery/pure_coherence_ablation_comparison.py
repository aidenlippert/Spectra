"""Exact matched-target fixed-recipe contribution of two pure-coherence operators."""
...
def main():
    ...
    if c['kind']!='hubbard_projector_extension_v19':raise ValueError('New energy version required')
    expected=copy.deepcopy(c);expected.pop('pure_coherence');expected['kind']='hubbard_projector_extension_v18';expected['penalized_lower']=a['penalized_lower']
    ...
    result={'accepted':True,'with_new_open_lower':energy['lower_per_site'],'without_new_open_lower':ablation['lower_per_site'],
            ...
            'scope':'Signed difference of accepted matched-target energy certificates, with only the two pure-coherence operators removed and scalar threshold recomputed. A positive value proves a benefit for this fixed recipe, not separation from a reoptimized older family.'}
