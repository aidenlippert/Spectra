...
def main():
    ...
    def evaluate(y):
        ...
        for key,a,ds,ph,q,ts,*_ in mats:
            matrix=a+np.einsum('i,ijk->jk',y[:2]-np.array(x[2:4],float),ds)+y[2]*ph+y[3]*q+np.diag(y[4:124]@ts)+np.einsum('i,ijk->jk',y[124:199],hop[key])
        ...
        if min(y[:4])>=0 and y[0]+y[1]<=2.5+1e-12 and max(abs(y[4:124]))<=8+1e-9 and max(abs(y[124:199]))<=2+1e-9 and (best is None or score>best[0]):best=(score,np.r_[y,minimum])
        return value,gradient
    point=y0[:-1].copy();initial_value,initial_gradient=evaluate(point)
    rng=np.random.default_rng(20260912);gradient_errors=[]
    for _ in range(2):
        direction=rng.normal(size=199);direction/=np.linalg.norm(direction);step=1e-8
    ...
    def coordinate_scale(y):
        ...
        errors=[]
        for j in (60,129):
            step=1e-9;direction=np.zeros(199);direction[j]=step
        ...
        direction=np.random.default_rng(20260912).normal(size=199);direction/=np.linalg.norm(direction);step=1e-9
    ...
    for tau in (1e-6,1e-7):
        ...
        try:
            ...
            bounds=np.array([(0,2.5)]*2+[(0,3)]*2+[(-8,8)]*120+[(-2,2)]*75)
            def constraints(z):
            ...
            outcome=minimize(lambda z:evaluate(origin+transform@z)[0],np.zeros(199),jac=lambda z:transform.T@evaluate(origin+transform@z)[1],method='SLSQP',constraints=[{'type':'ineq','fun':constraints,'jac':lambda z:constraint_jac}],options={'maxiter':120,'ftol':1e-13})
    ...
    for key,a,ds,ph,q,ts,*_ in mats:
        matrix=a+np.einsum('i,ijk->jk',atom_point[:2]-np.array(x[2:4],float),ds)+atom_point[2]*ph+atom_point[3]*q+np.diag(atom_point[4:124]@ts)+np.einsum('i,ijk->jk',atom_point[124:199],hop[key])
    ...
    fresh_action=[actions()]+[spin_actions({label:1}) for label in LABELS]+[spectator_actions({label:1}) for label in SPECTATORS]+[pair_actions({label:1}) for label in PAIRS]+[two_actions({label:1}) for label in TWO]+[three_actions({label:1}) for label in THREE]+[coherent_actions({label:1}) for label in COHERENT]+[pure_actions({label:1}) for label in PURE]
    minimum=(float('inf'),None);difference=0.
    for key,a,ds,ph,q,ts,cols,*_ in mats:
        k=fresh[key][1];scale=np.sqrt([sum(v*v for v in col.values()) for col in cols]);direct=np.array(k,float)/scale[:,None]/scale[None,:]+np.einsum('i,ijk->jk',np.array(y[124:199],float),np.array([projected_matrix(cols,item) for item in fresh_action],float))/scale[:,None]/scale[None,:]
        correction=np.array([float(local_value(next(iter(col)),frozen)+local_value(next(iter(col)),word)) for col in cols])
        affine=np.einsum('i,ijk->jk',np.array(y[124:199],float),hop[key])+a+np.einsum('i,ijk->jk',np.array(y[:2],float)-np.array(x[2:4],float),ds)+np.diag(np.array(y[4:124],float)@ts)
    ...
    result={'accepted':False,'matrix_evaluations':calls,'optimizer_success':False if outcome is None else bool(outcome.success),'optimizer_message':'hard evaluation budget' if outcome is None else str(outcome.message),'proposed_periodic_lower':str(lower),'proposed_periodic_lower_float':float(lower),'max_fresh_affine_difference':difference,'hopping_telescope':str(y[124]),'spin_telescope':out['spin_telescope'],'spectator_hopping':out['spectator_hopping'],'pair_transfer':out['pair_transfer'],'two_spectator_hopping':out['two_spectator_hopping'],'coherent_projector':out['coherent_projector'],'pure_coherence':out['pure_coherence'],'three_spectator_hopping':out['three_spectator_hopping'],'spin_word_components':len(out['spin_word_telescope']),'shape_indices':indices,'seconds':time.monotonic()-started,'source_sha256':{str(p.resolve().relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path(__file__),Path(__file__).with_name("signed_charge_numeric.py"),Path(__file__).with_name("spin_word_numeric.py"),ROOT/"experiments/marginal_spin_word_telescope.py",ROOT/"experiments/marginal_pure_coherence.py",ROOT/"experiments/marginal_hopping_telescope.py",ROOT/"experiments/marginal_spin_telescope.py",ROOT/"experiments/marginal_spectator_hopping.py",ROOT/"experiments/marginal_pair_transfer.py",ROOT/"experiments/marginal_two_spectator_hopping.py",ROOT/"experiments/marginal_three_spectator_hopping.py",ROOT/"experiments/marginal_coherent_projector_telescope.py",args.seed,candidates_path]},'scope':'Nonaccepting199-coefficient full-spectrum search:120 complete diagonal spin-word directions,75 offdiagonal directions,2 hopping profiles and2 penalties. Frozen legacy diagonal fields remain in the base but their full span is contained in the120 new coordinates. Bounds +/-8 for word coefficients and +/-2 for offdiagonal coefficients; <=500 spectral evaluations. Gradient/Hessian checks and fresh exact-physical reconstruction precede scalar proposal. Exact PSD acceptance remains separate.'}
