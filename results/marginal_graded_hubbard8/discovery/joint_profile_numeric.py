...
from experiments.marginal_charge_square_pairs import LABELS as SQUARE_LABELS, local_value as square_value
from experiments.marginal_charge_indicator_telescope import LABELS as INDICATOR_LABELS, local_value as indicator_value
...
def build(x,V=F(1,2),W=None,range_profile=None,quadratic_terms=None,square_terms=None,indicator_terms=None):
    u,t,v=profiles(x,V)
    ...
    for key,(states,matrix,cols) in data.items():
        diagonal=[]
        for col in cols:
            values={diagonal_value(s,density2)+(quadratic_value(s,quadratic_terms) if quadratic_terms else 0)+(square_value(s,square_terms) if square_terms else 0)+(indicator_value(s,indicator_terms) if indicator_terms else 0) for s in col}
...
def main():
    ...
    parser.add_argument('--quadratic-charge',action='store_true')
    parser.add_argument('--charge-square-pairs',action='store_true')
    parser.add_argument('--full-charge-indicators',action='store_true')
    ...
    if args.quadratic_charge and not args.free_range_two_profile:raise ValueError('Quadratic completion requires free range-two profile')
    if args.charge_square_pairs and not args.quadratic_charge:raise ValueError('Charge-square pairs require quadratic mode')
    square_count=len(SQUARE_LABELS) if args.charge_square_pairs else 0
    if args.full_charge_indicators and not args.charge_square_pairs:raise ValueError('Full indicators require charge-square pairs')
    indicator_count=len(INDICATOR_LABELS) if args.full_charge_indicators else 0
    diagonal_count=len(Ys)+int(args.free_range_two_profile)+int(args.quadratic_charge)+square_count+indicator_count
    ...
    for key,(_,K,cols) in original.items():
        ...
        if args.quadratic_charge:diagonals.append([float(quadratic_value(next(iter(col)),{'0,3':F(1)})) for col in cols])
        if args.charge_square_pairs:
            diagonals.extend([[float(square_value(next(iter(col)),{label:F(1)})) for col in cols] for label in SQUARE_LABELS])
        if args.full_charge_indicators:
            diagonals.extend([[float(indicator_value(next(iter(col)),{label:F(1)})) for col in cols] for label in INDICATOR_LABELS])
    ...
    for row in cases:
        ...
        if args.telescoping:
            ...
            if args.quadratic_charge:
                if set(cert.get('quadratic_charge_telescope',{}))-{'0,3'}:raise ValueError('Unsupported quadratic seed span')
                seed_taus.append(float(F(cert.get('quadratic_charge_telescope',{}).get('0,3',0))))
            if args.charge_square_pairs:
                seed_taus.extend(float(F(cert.get('charge_square_pair_telescope',{}).get(label,0))) for label in SQUARE_LABELS)
            if args.full_charge_indicators:
                seed_taus.extend(float(F(cert.get('higher_charge_indicator_telescope',{}).get(label,0))) for label in INDICATOR_LABELS)
        ...
        indicator_terms=dict(zip(INDICATOR_LABELS,taus[-indicator_count:])) if indicator_count else None
        if indicator_count:taus=taus[:-indicator_count]
        square_terms=dict(zip(SQUARE_LABELS,taus[-square_count:])) if square_count else None
        if square_count:taus=taus[:-square_count]
        quadratic_coefficient=taus.pop() if args.quadratic_charge else F(0)
        quadratic_terms={'0,3':quadratic_coefficient} if args.quadratic_charge else None
        ...
        fresh=build(rational[:6],args.target_v,args.target_w,range_profile,quadratic_terms,square_terms,indicator_terms);minimum=(float('inf'),None);max_difference=0.
        for key,A,deriv,P,Q,T in mats:
            ...
            if args.free_range_two_profile:affine=affine+np.diag(float(free_shift)*T[len(Ys)])
            if args.quadratic_charge:affine=affine+np.diag(float(quadratic_coefficient)*T[len(Ys)+int(args.free_range_two_profile)])
            if square_count:affine=affine+np.diag(np.array(list(square_terms.values()),float)@T[len(Ys)+int(args.free_range_two_profile)+int(args.quadratic_charge):len(Ys)+int(args.free_range_two_profile)+int(args.quadratic_charge)+square_count])
            if indicator_count:affine=affine+np.diag(np.array(list(indicator_terms.values()),float)@T[-indicator_count:])
        ...
        if args.quadratic_charge:
            result['kind']='hubbard_projector_extension_v7'
            result['quadratic_charge_telescope']={'0,3':str(quadratic_coefficient)}
        if args.charge_square_pairs:
            result['kind']='hubbard_projector_extension_v8'
            result['charge_square_pair_telescope']={key:str(value) for key,value in square_terms.items() if value}
        if args.full_charge_indicators:
            result['kind']='hubbard_projector_extension_v9'
            result['higher_charge_indicator_telescope']={key:str(value) for key,value in indicator_terms.items() if value}
        ...
        if args.quadratic_charge:item.update(complete_quadratic_charge_family=True,quadratic_charge_coefficient=str(quadratic_coefficient))
        if args.charge_square_pairs:item.update(charge_square_pair_coefficients={key:str(value) for key,value in square_terms.items()})
        if args.full_charge_indicators:item.update(higher_indicator_coefficients={key:str(value) for key,value in indicator_terms.items()})
