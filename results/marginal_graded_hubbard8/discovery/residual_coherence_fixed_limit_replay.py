...
from residual_coherence_obstruction import ROOT, BASE, OLD_MODULES, pure_coherence, read_accepted, hashes
...
def fixed_expectation(c):
    if c.get('kind') != 'hubbard_projector_extension_v19' or any(F(c['target'][k]) != v for k, v in [('U', 4), ('t', 1), ('V', F(1, 2))]):
        raise ValueError('Matched accepted ENERGYv19 recipe required')
    ...
    combined = [dict(physical[state]) for state in range(4096)]
...
def replay(c, proposal):
    if proposal.get('kind') != 'residual_coherence_fixed_family_proposal_v1':
    ...
    return {'accepted': True, 'mixture_sources': len(states), 'mixture_trace': '1',
            ...
            'scope': 'An exact ceiling only for the two new coefficients with all ENERGYv19 fields frozen. This does not cap reoptimization of older fields, the full enlarged family, or physical ground energy.'}
