...
def minimum(c):
    ...
    for key,fn in [('spin_telescope',spin_actions),('spectator_hopping',spectator_actions),('pair_transfer',pair_actions),('two_spectator_hopping',two_actions),('three_spectator_hopping',three_actions)]:
        action.append((F(1),fn({k:F(v) for k,v in c.get(key,{}).items()})))
    ...
    terms = [(fn,{k:F(v) for k,v in c.get(key,{}).items()}) for key,fn in
