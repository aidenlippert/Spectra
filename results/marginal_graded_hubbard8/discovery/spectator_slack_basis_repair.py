...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('directory',type=Path);parser.add_argument('--ledger',type=Path,required=True);parser.add_argument('--scale',type=float,default=.002);parser.add_argument('--determinant-anchors',action='store_true');parser.add_argument('--pricing-rounds',type=int,default=0);parser.add_argument('--pricing-radius',type=float,default=0);parser.add_argument('--pricing-eigenvectors',type=int,choices=(1,2),default=1);args=parser.parse_args();out=args.directory
    ...
    with gzip.open(args.ledger,'rt') as handle:ledger=json.load(handle)
    states=[(item['vector'],item['norm'],item['coeff'],tuple(item['sector']) if item['sector'] is not None else None) for item in ledger['states']]
    rows=[np.array(row) for row in ledger['rows']];energies=ledger['energies']
    assert len(states)==len(rows)==len(energies)<=13000
    exact={key:(k,derivatives,cols,ts) for key,a,ds,ph,q,ts,cols,k,derivatives in mats}
    ...
    arr=np.array(rows).T;rhs=[F(1)]+[F(0)]*82+[th,tj];pricing=[]
    assert list(map(F,ledger['rhs']))==rhs
    ...
    diagnostic={'accepted':False,'proposal_written':False,'lp_tolerance':1e-10,'support_threshold':1e-14,'scale':args.scale,'pricing':pricing,'pricing_eigenvectors':args.pricing_eigenvectors,'pricing_radius':args.pricing_radius,'determinant_anchors':args.determinant_anchors,'states':len(states),'active_sectors':list(exact),'status':result.message,'seconds':time.monotonic()-started}
    diagnostic['slack_attempts']=[]
    for extra_slacks in ((0,1),(0,),(1,),()):
        if result.success:
            values=list(result.x)+list(np.array(rhs[83:],float)-arr[83:]@result.x);support=[i for i,v in enumerate(values) if v>1e-14]
            for slack in extra_slacks:
                if len(states)+slack not in support:support.append(len(states)+slack)
            if len(support)>85:continue
            diagnostic.update(numerical_family_upper=float(result.fun)/5,basis_size=len(support))
            erows=[];eenergies=[]
        ...
        diagnostic['slack_attempts'].append({'added_slacks':extra_slacks,'basis_size':diagnostic.get('basis_size'),'proposal_written':diagnostic['proposal_written'],'failure':diagnostic.get('exact_basis_failure')})
        if diagnostic['proposal_written']:break
