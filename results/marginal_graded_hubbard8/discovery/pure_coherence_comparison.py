...
def match(new, old, family):
    if new['kind'] != 'hubbard_projector_extension_v19' or old['kind'] != 'hubbard_projector_extension_v18':
        raise ValueError('Explicit spin-word extension and preceding full spin-word certificate required')
...
def main():
    ...
    if not family_receipt.get('full_spin_word'):
    ...
    result = {'accepted':True,'strict_family_separation':lower>cap,'periodic_pure_coherence_lower':str(lower),
              ...
              'scope':'Exact lower improvement at fixed target, size, projector sources, ratio, ceilings and sparse span. A positive signed separation proves the new lower exceeds the preceding full spin-word family ceiling; a nonpositive value does not establish such separation. No enlarged-family optimality or physical interpretation of family ceilings.'}
