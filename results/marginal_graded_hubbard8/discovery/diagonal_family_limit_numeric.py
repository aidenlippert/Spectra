...
def main():
    global OUT
    parser=argparse.ArgumentParser();parser.add_argument('--directory',type=Path,default=OUT);parser.add_argument('--range-two',action='store_true');parser.add_argument('--free-range-two-profile',action='store_true');parser.add_argument('--quadratic-charge',action='store_true');parser.add_argument('--charge-square-pairs',action='store_true');parser.add_argument('--full-charge-indicators',action='store_true');parser.add_argument('--perturbation-scale',type=float,default=.002)
    args=parser.parse_args();OUT=args.directory
    if not 0 < args.perturbation_scale <= .2:raise ValueError('Bounded dual perturbation scale required')
    ...
    if args.full_charge_indicators and not args.charge_square_pairs:raise ValueError('Full indicators require square-pair mode')
    if bool(c.get('higher_charge_indicator_telescope'))!=args.full_charge_indicators:raise ValueError('Indicator mode must match energy certificate')
    indicator_terms=None
    if args.full_charge_indicators:
        from experiments.marginal_charge_indicator_telescope import LABELS as INDICATOR_LABELS, coefficients as indicator_coefficients, local_value as indicator_value
        indicator_terms=indicator_coefficients(c['higher_charge_indicator_telescope'])
    range_profile=None
    if args.range_two:
    ...
    diagonal_count=9+int(args.free_range_two_profile)+int(args.quadratic_charge)+4*int(args.charge_square_pairs)+6*int(args.full_charge_indicators)
    ...
    original = build(x,W=W,range_profile=range_profile,quadratic_terms=quadratic_terms,square_terms=square_terms,indicator_terms=indicator_terms)
    perturbed = []
    for j in range(6):
        point = x.copy()
        point[j] += F(1, 100)
        perturbed.append(build(point,W=W,range_profile=range_profile,quadratic_terms=quadratic_terms,square_terms=square_terms,indicator_terms=indicator_terms))
    ...
    for key, (_, k, cols) in original.items():
        ...
        if args.full_charge_indicators:ts=np.vstack([ts,[[int(indicator_value(next(iter(col)),{label:F(1)})) for col in cols] for label in INDICATOR_LABELS]])
        ...
        directions = [np.zeros(direction_count)]+[sign*np.eye(direction_count)[j]*args.perturbation_scale for j in range(direction_count) for sign in (-1, 1)]
        directions += [rng.normal(size=direction_count)*args.perturbation_scale for _ in range(16)]
    ...
    diagnostic = {'accepted': False, 'perturbation_scale':args.perturbation_scale, 'states': len(states), 'active_sectors': list(exact_data), 'status': result.message, 'seconds': time.monotonic()-started}
    if result.success:
        ...
        if len(support) == row_count:
            ...
            if args.full_charge_indicators:proposal['kind']='joint_diagonal_full_charge_indicator_family_proposal_v1'
