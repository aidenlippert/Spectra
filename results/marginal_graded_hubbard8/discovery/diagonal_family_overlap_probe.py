...
def main():
    ...
    selected=[]
    for shape in c['diagonal_shapes']:
        exact={int(s):F(v) for s,v in shape.items()}
        matches=[i for i,row in enumerate(candidates) if {int(s):F(v) for s,v in row['diagonal'].items()}==exact]
        if len(matches)!=1:raise ValueError('Accepted shape is missing or ambiguous in the catalog')
