...
def main():
    ...
    if residual_coherence:source['kind']='joint_diagonal_range2_family_limit_v16'
    ...
    if ('residual_coherence' in energy or energy.get('kind') == 'hubbard_projector_extension_v20') and not residual_coherence:
        raise ValueError('Residual-coherence energy requires a matching enlarged family cap')
    ...
    receipt = {'accepted': True, 'free_range_two_profile':free_profile, 'full_quadratic_charge':full_quadratic, 'charge_square_pairs':square_pairs, 'full_charge_indicators':full_indicators, 'full_signed_charge':full_signed, 'hopping_telescope':coherent, 'spin_telescope':spin, 'spectator_hopping':spectator, 'pair_transfer':pair_transfer, 'two_spectator_hopping':two_spectator, 'three_spectator_hopping':three_spectator, 'coherent_projector':coherent_projector, 'full_spin_word':full_spin_word, 'pure_coherence':pure_coherence, 'residual_coherence':residual_coherence, 'family_replay': result, 'accepted_periodic_lower': str(lower),
    ...
    if residual_coherence:receipt['scope']+=' Both residual-coherence moments are exactly zero.'
