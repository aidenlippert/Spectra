...
# A12000-bit matrix denominator can produce <=24024-bit squared-norm outputs.
...
def reconstruct(mixture):
    if not 1<=len(mixture)<=211:raise ValueError('At most211 physical mixture sources required')
    sources=[]
    for item in mixture:
...
def main():
    ...
    if not r.get('accepted') or (not r.get('full_spin_word') or not r.get('pure_coherence') or not r.get('residual_coherence')):raise ValueError('Accepted full spin-word family required')
    ...
    closed={}
    for label,vector in [('346,409,1',{346:1,409:1}),('314,614,1',{314:1,614:1}),('358,409,1',{358:1,409:1}),('103,358,1',{103:1,358:1})]:
        value=F(quadratic(difference,vector),denominator*sum(a*a for a in vector.values()))
        if value:raise ValueError('New pure-coherence projector did not close in complete RDM')
        closed[label]=str(value)
    if not difference:
        ...
        result={'accepted':True,'stationary_extension_refuted':False,'five_site_overlap_agrees':True,'closed_projector_moments':closed,
    ...
    for module in tuple(sys.modules.values()):
        name=getattr(module,'__file__',None)
        if name and str(Path(name).resolve()).startswith(str(ROOT/'experiments')+'/'):files.add(Path(name).resolve())
    result={'accepted':True,'stationary_extension_refuted':True,'closed_projector_moments':closed,'symmetrization':'Equal eight-image projective orbit under six-site particle-hole, spin flip and reflection.',
