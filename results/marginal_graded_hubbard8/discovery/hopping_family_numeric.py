...
def main():
    ...
    result=linprog(energies,A_eq=arr[:65],b_eq=np.array(rhs[:65],float),A_ub=arr[65:],b_ub=np.array(rhs[65:],float),bounds=(0,None),method='highs',options={'dual_feasibility_tolerance':1e-10,'primal_feasibility_tolerance':1e-10})
    pricing=[]
    seed_dual=np.r_[float(F(c['penalized_lower'])),-np.array(x[2:4],float),-np.array(taus),-float(gamma),-float(alpha),-float(beta)]
    for iteration in range(args.pricing_rounds):
        ...
        for key,a,ds,ph,q,ts,cols,k,derivatives in mats:
            scale=np.sqrt([sum(v*v for v in col.values()) for col in cols]);h=np.array(hop[key],float)/scale[:,None]/scale[None,:]
            price=a-np.einsum('i,ijk->jk',np.array(x[2:4],float)+dual[1:3],ds)-np.diag(dual[3:64]@ts)-dual[64]*h-dual[65]*ph-dual[66]*q
    ...
    if result.success:
        values=list(result.x)+list(np.array(rhs[65:],float)-arr[65:]@result.x);support=[i for i,v in enumerate(values) if v>1e-14];diagnostic.update(numerical_family_upper=float(result.fun)/5,basis_size=len(support))
        erows=[];eenergies=[]
