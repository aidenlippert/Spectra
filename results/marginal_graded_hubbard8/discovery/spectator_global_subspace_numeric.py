...
def main():
    ...
    def enrich(y):
        ...
        for e,key,v in spectra:
            ...
            basis[key]=u[:,:n]
        if len(basis)>94 or sum(v.shape[1] for v in basis.values())>1000:raise RuntimeError('total compressed order cap1000 reached')
    evaluate(center)
