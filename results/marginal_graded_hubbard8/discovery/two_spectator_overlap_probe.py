"""Exact two-spectator charge-hopping consistency beyond the pair family."""
...
def reflected(label):
    i,j,k,l,p,q = label
    return (4-j,4-i,4-l,4-k,q,p)


LABELS = tuple(label for i in range(5) for j in range(i+1,5)
    for k,l in combinations([s for s in range(5) if s not in (i,j)],2)
    for p,q in (((1,1),(2,2)) if (j-i)%2 else ((1,2),(2,1)))
    for label in [(i,j,k,l,p,q)] if label < reflected(label))


def components(label):
    i,j,k,l,p,q = label
    return [(i,j,k,l,p,q,1),(*reflected(label),-1),
            (i+1,j+1,k+1,l+1,p,q,-1),
            (5-j,5-i,5-l,5-k,q,p,1)]
...
def operator(label):
    ...
    for state in range(4096):
        ...
        row = {}
        for i,j,k,l,p,q,sign in terms:
            weight = sign*charges[k]**p*charges[l]**q
            for a,b in ((i,j),(j,i)):
                for spin in (0,1):
                    target = apply_word(((1,2*a+spin),(0,2*b+spin)),state)
                    if target is not None:
                        s,phase = target; row[s] = row.get(s,0)+weight*phase
        result.append({s:a for s,a in row.items() if a})
    for s,row in enumerate(result):
        ...
        for fn in (lambda s:_reflection(s,6),_particlehole,_spinflip):
            ...
            if right != {t:phase*a for t,a in result[us].items()}:
                raise ValueError('Two-spectator symmetry failed')
    for sites in (8,9,10,17):
        totals = {}
        for offset in range(sites):
            for i,j,k,l,p,q,sign in terms:
                pair = tuple(sorted(((i+offset)%sites,(j+offset)%sites)))
                spectators = tuple(sorted((((k+offset)%sites,p),((l+offset)%sites,q))))
...
def main():
    ...
    if not r.get('accepted') or not r.get('pair_transfer'):
        raise ValueError('Accepted pair-transfer family required')
    for name,expected in r['source_sha256'].items():
        if hashlib.sha256((ROOT/name).read_bytes()).hexdigest()!=expected:
            raise ValueError('Stale accepted source: '+name)
    if len(LABELS)!=30:raise ValueError('Expected full30-direction two-spectator basis')
    base_norm=hopping_norm_bound()
    data=[operator(label) for label in LABELS]
    old=[constant_operator(*pair) for pair in ((0,1),(0,3),(1,2))]+[
        old_actions({label:1}) for label in OLD_LABELS]
    ...
    # including the two variable nearest-neighbor profiles.
    ...
    result={'accepted':True,'directions':30,'violated_directions':sum(m['violated'] for m in moments),
            ...
            'scope':'Exact necessary stationary consistency tests for30 PH-even reflected two-spectator charge-hopping telescopes. Full-Fock CAR, Hermiticity, reflection/PH/spin-flip symmetry and periodic cancellation checked. Each term is a product of two disjoint charge powers (norm<=1) and spin-summed hopping (norm2); triangle norm bound8. Restricted integer Gram measures independence modulo all17 preceding hopping directions; other preceding operators have no entries on this restriction. Nonzero symmetry-invariant moments exclude a stationary extension of this particular averaged local mixture. No stronger energy bound, enlarged-family optimum or general representability claim.'}
    (folder/'two_spectator_overlap.json').write_text(json.dumps(result,indent=2)+'\n')
    largest=sorted(moments,key=lambda m:abs(m['moment_float']),reverse=True)[:4]
    print(json.dumps({'accepted':True,'directions':30,'violated':result['violated_directions'],
