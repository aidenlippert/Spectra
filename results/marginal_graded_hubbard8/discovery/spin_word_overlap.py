...
def reconstruct(mixture):
    if not 1<=len(mixture)<=207:raise ValueError('At most207 physical mixture sources required')
    sources=[]
    for item in mixture:
...
def main():
    ...
    if not r.get('accepted') or not r.get('full_spin_word'):raise ValueError('Accepted full spin-word family required')
