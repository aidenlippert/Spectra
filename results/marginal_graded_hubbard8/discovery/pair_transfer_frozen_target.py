...
def minimum(c):
    ...
    for key,fn in [('spin_telescope',spin_actions),('spectator_hopping',spectator_actions),('pair_transfer',pair_actions)]:
        action.append((F(1),fn({k:F(v) for k,v in c[key].items()})))
