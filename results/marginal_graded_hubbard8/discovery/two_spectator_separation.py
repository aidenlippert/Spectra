"""Exact separation from the entire preceding fixed pair-transfer family."""
...
def match(new, old, family):
    if new['kind'] != 'hubbard_projector_extension_v15' or old['kind'] != 'hubbard_projector_extension_v14':
        raise ValueError('Explicit two-spectator extension and preceding pair certificate required')
...
def main():
    ...
    result = {'accepted':True,'periodic_two_spectator_lower':str(lower),
              ...
              'scope':'New accepted two-spectator lower strictly exceeds a freshly replayed ceiling for every certificate in the preceding fixed pair-transfer family. Same physical target, size, projector sources, ratio, ceilings and sparse span. No optimality claim for the enlarged family or physical interpretation of the old family ceiling.'}
