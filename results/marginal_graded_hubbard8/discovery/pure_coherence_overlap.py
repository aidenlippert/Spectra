...
def reconstruct(mixture):
    if not 1<=len(mixture)<=209:raise ValueError('At most209 physical mixture sources required')
    sources=[]
    for item in mixture:
...
def main():
    ...
    closed={}
    for label,vector in [('346,409,1',{346:1,409:1}),('314,614,1',{314:1,614:1})]:
    ...
    if not difference:
        ...
        result={'accepted':True,'stationary_extension_refuted':False,'five_site_overlap_agrees':True,'pure_coherence_projector_moments':closed,
    ...
    for module in tuple(sys.modules.values()):
        name=getattr(module,'__file__',None)
        if name and str(Path(name).resolve()).startswith(str(ROOT/'experiments')+'/'):files.add(Path(name).resolve())
    result={'accepted':True,'stationary_extension_refuted':True,'pure_coherence_projector_moments':closed,'symmetrization':'Equal eight-image projective orbit under six-site particle-hole, spin flip and reflection.',
