...
def main():
    ...
    # Save the finite candidate set so exact-basis follow-up need not rerun
    # physical sampling or spectral pricing. This ledger is nonaccepting.
    ...
    diagnostic['basis_attempts']=[]
    for row_scale in (100.,10000.,1000000.):
        eq_scale=np.r_[1.,np.full(82,row_scale)]
        result=linprog(energies,A_eq=arr[:83]*eq_scale[:,None],b_eq=np.array(rhs[:83],float)*eq_scale,A_ub=arr[83:],b_ub=np.array(rhs[83:],float),bounds=(0,None),method='highs',options={'dual_feasibility_tolerance':1e-10,'primal_feasibility_tolerance':1e-10})
        diagnostic['exact_basis_failure']=None
        if result.success:
        ...
        diagnostic['basis_attempts'].append({'row_scale':row_scale,'status':result.message,'basis_size':diagnostic.get('basis_size'),'proposal_written':diagnostic['proposal_written'],'exact_basis_failure':diagnostic.get('exact_basis_failure')})
