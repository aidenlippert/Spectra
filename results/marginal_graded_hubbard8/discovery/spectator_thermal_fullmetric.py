...
def main():
    ...
    def evaluate(y):
        ...
        if min(y[:4])>=0 and y[0]+y[1]<=2.5+1e-12 and max(abs(y[13:65]))<=8+1e-9 and max(abs(np.r_[y[4:13],y[65:84]]))<=2+1e-9 and (best is None or score>best[0]):best=(score,np.r_[y,minimum])
    ...
    def coordinate_scale(y):
        ...
        direction=np.random.default_rng(20260912).normal(size=84);direction/=np.linalg.norm(direction);step=1e-9
    ...
    for tau in (1e-6,1e-7):
        ...
        try:
            ...
            bounds=np.array([(0,2.5)]*2+[(0,3)]*2+[(-2,2)]*9+[(-8,8)]*52+[(-2,2)]*19)
            def constraints(z):
            ...
            outcome=minimize(lambda z:evaluate(origin+transform@z)[0],np.zeros(84),jac=lambda z:transform.T@evaluate(origin+transform@z)[1],method='SLSQP',constraints=[{'type':'ineq','fun':constraints,'jac':lambda z:constraint_jac}],options={'maxiter':120,'ftol':1e-13})
