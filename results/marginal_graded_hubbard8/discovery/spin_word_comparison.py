...
def match(new, old, family):
    if new['kind'] != 'hubbard_projector_extension_v18' or old['kind'] != 'hubbard_projector_extension_v17':
        raise ValueError('Explicit spin-word extension and preceding coherent-projector certificate required')
...
def main():
    ...
    if not family_receipt.get('coherent_projector'):
        raise ValueError('Preceding coherent-projector family cap required')
    ...
    result = {'accepted':True,'strict_family_separation':lower>cap,'periodic_spin_word_lower':str(lower),
              ...
              'scope':'Exact lower improvement at fixed target, size, projector sources, ratio, ceilings and sparse span. A positive signed separation proves the new lower exceeds the preceding full coherent-projector family ceiling; a nonpositive value does not establish such separation. No enlarged-family optimality or physical interpretation of family ceilings.'}
