...
def solve_basis(matrix,rhs):
    ...
    if not 1<=n<=m<=71 or any(len(row)!=n for row in matrix):raise ValueError('Bounded rectangular dual basis required')
...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('directory',type=Path);parser.add_argument('--scale',type=float,default=.002);parser.add_argument('--determinant-anchors',action='store_true');parser.add_argument('--pricing-rounds',type=int,default=0);parser.add_argument('--pricing-radius',type=float,default=0);args=parser.parse_args();out=args.directory
    ...
    action=[actions()]+[spin_actions({label:1}) for label in LABELS];hop={key:[projected_matrix(cols,item) for item in action] for key,a,ds,ph,q,ts,cols,*_ in mats}
    gamma=[F(c['hopping_telescope'])]+[F(c['spin_telescope'].get(label,0)) for label in LABELS]
    ...
    for key,a,ds,ph,q,ts,cols,k,derivatives in mats:
        ...
        directions=[np.zeros(68)]+[sign*np.eye(68)[j]*args.scale for j in range(68) for sign in (-1,1)]+[rng.normal(size=68)*args.scale for _ in range(16)]
        for delta in directions:
            _,vectors=eigh(m+np.einsum('i,ijk->jk',delta[:2],ds)+np.diag(delta[2:63]@ts)+np.einsum('i,ijk->jk',delta[63:68],h),subset_by_index=[0,0]);v=vectors[:,0]/scale;coeff=[round(float(z/max(abs(v)))*10**8) for z in v]
    ...
    arr=np.array(rows).T;rhs=[F(1)]+[F(0)]*68+[th,tj]
    result=linprog(energies,A_eq=arr[:69],b_eq=np.array(rhs[:69],float),A_ub=arr[69:],b_ub=np.array(rhs[69:],float),bounds=(0,None),method='highs',options={'dual_feasibility_tolerance':1e-10,'primal_feasibility_tolerance':1e-10})
    pricing=[]
    ...
    for iteration in range(args.pricing_rounds):
        ...
        for key,a,ds,ph,q,ts,cols,k,derivatives in mats:
            scale=np.sqrt([sum(v*v for v in col.values()) for col in cols]);h=np.array(hop[key],float)/scale[:,None]/scale[None,:]
            price=a-np.einsum('i,ijk->jk',np.array(x[2:4],float)+dual[1:3],ds)-np.diag(dual[3:64]@ts)-np.einsum('i,ijk->jk',dual[64:69],h)-dual[69]*ph-dual[70]*q
            ev,vectors=eigh(price,subset_by_index=[0,0]);reduced=float(ev[0])-dual[0];lowest=min(lowest,reduced)
            if reduced>=-1e-9:continue
            v=vectors[:,0]/scale;coeff=[round(float(z/max(abs(v)))*10**8) for z in v]
            vector={str(s):z*b for z,col in zip(coeff,cols) if z for s,b in col.items()};norm=sum(v*v for v in vector.values());w=np.array(coeff,float)*scale/np.sqrt(float(norm));d=np.array([w@matrix@w for matrix in ds]);row=np.r_[1,d,ts@(w*w),[w@matrix@w for matrix in h],w@ph@w,w@q@w]
        ...
        arr=np.array(rows).T
        result=linprog(energies,A_eq=arr[:69],b_eq=np.array(rhs[:69],float),A_ub=arr[69:],b_ub=np.array(rhs[69:],float),bounds=(0,None),method='highs',options={'dual_feasibility_tolerance':1e-10,'primal_feasibility_tolerance':1e-10})
    diagnostic={'accepted':False,'proposal_written':False,'lp_tolerance':1e-10,'support_threshold':1e-14,'scale':args.scale,'pricing':pricing,'pricing_radius':args.pricing_radius,'determinant_anchors':args.determinant_anchors,'states':len(states),'active_sectors':list(exact),'status':result.message,'seconds':time.monotonic()-started}
    if result.success:
        values=list(result.x)+list(np.array(rhs[69:],float)-arr[69:]@result.x);support=[i for i,v in enumerate(values) if v>1e-14];diagnostic.update(numerical_family_upper=float(result.fun)/5,basis_size=len(support))
        erows=[];eenergies=[]
        for index in support:
            if index>=len(states):erows.append([F(int(j==69+index-len(states))) for j in range(71)]);eenergies.append(F(0));continue
