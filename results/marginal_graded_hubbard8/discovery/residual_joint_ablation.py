"""Untrusted frozen-coefficient ablation; remove only the two residual-coherence telescopes."""
from pathlib import Path
from fractions import Fraction as F
import argparse,hashlib,json,math,sys
from residual_joint_frozen_target import ROOT,minimum
...
def main():
    ...
    if c.get('kind')!='hubbard_projector_extension_v20' or not c.get('residual_coherence'):
        raise ValueError('Explicit new-operator source required')
    c.pop('residual_coherence');c['kind']='hubbard_projector_extension_v19'
    ...
    files={Path(__file__).resolve(),args.seed.resolve(),Path(sys.modules['residual_joint_frozen_target'].__file__).resolve()}
    ...
    result={'accepted':False,'numerical_minimum':value,'proposed_threshold':c['penalized_lower'],
            ...
            'scope':'Removed only residual_coherence and lowered schema to v19; all other coefficients fixed, scalar threshold reproposed. Requires exact replay.'}
