...
def main():
    ...
    def evaluate(y):
        ...
        if min(y[:4])>=0 and y[0]+y[1]<=2.5+1e-12 and (best is None or score>best[0]):best=(score,np.r_[y,minimum])
        return value,gradient
    point=y0[:-1].copy();initial_value,initial_gradient=evaluate(point)
    rng=np.random.default_rng(20260912);gradient_errors=[]
    for _ in range(2):
        direction=rng.normal(size=84);direction/=np.linalg.norm(direction);step=1e-8
    ...
    def coordinate_scale(y):
        ...
        for key,ev,vectors,ds,ph,q,ts in latest_spectra:
            ...
            for i in np.flatnonzero(weights>1e-14):
                ...
                curvature+=weights[i]*projected[i]**2/tau
                gaps=ev[i+1:]-ev[i]
                ...
                curvature+=2*np.sum(coefficients[:,None]*projected[i+1:]**2,axis=0)
        mean=-5*g;mean[2]+=float(theta_h);mean[3]+=float(theta_j)
        diagonal=(curvature-mean*mean/tau)/5
        # Check two coordinate curvatures against a derivative difference.
        errors=[]
        for j in (60,70):
            step=1e-9;direction=np.zeros(84);direction[j]=step
        ...
        scales=np.clip(1/np.sqrt(np.maximum(diagonal,1e-10)),1e-3,100.)
        curvature_checks.append({'temperature':tau,'minimum_curvature':float(min(diagonal)),'maximum_curvature':float(max(diagonal)),'scale_min':float(min(scales)),'scale_max':float(max(scales)),'finite_difference_errors':errors})
        return scales
    outcome=None
    for tau in (1e-6,1e-7):
        before=calls
        try:
            origin=point.copy();scales=coordinate_scale(point)
            bounds=[((lo-z)/scale,(hi-z)/scale) for (lo,hi),z,scale in zip([(0,2.5)]*2+[(0,3)]*2+[(-2,2)]*9+[(-8,8)]*52+[(-2,2)]*19,origin,scales)]
            outcome=minimize(lambda z:evaluate(origin+scales*z)[0],np.zeros(84),jac=lambda z:evaluate(origin+scales*z)[1]*scales,method='SLSQP',bounds=bounds,constraints=[{'type':'ineq','fun':lambda z:2.5-sum((origin+scales*z)[:2]),'jac':lambda z:np.r_[-scales[:2],np.zeros(82)]}],options={'maxiter':120,'ftol':1e-13})
            point=origin+scales*outcome.x
    ...
    for key,a,ds,ph,q,ts,*_ in mats:
        matrix=a+np.einsum('i,ijk->jk',atom_point[:2]-np.array(x[2:4],float),ds)+atom_point[2]*ph+atom_point[3]*q+np.diag(atom_point[4:65]@ts)+np.einsum('i,ijk->jk',atom_point[65:84],hop[key])
