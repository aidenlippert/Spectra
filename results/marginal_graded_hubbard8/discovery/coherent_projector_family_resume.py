...
from coherent_projector_fraction_free import solve as solve_basis
...
def main():
    ...
    if list(map(F,ledger['rhs'])) not in ([F(1)]+[F(0)]*134+[th,tj],[F(1)]+[F(0)]*136+[th,tj]):raise ValueError('Resume moment target differs')
    ...
    if len(ledger['rhs'])==137:
        # Existing ledger is an untrusted proposal; append two moments from its actual vectors.
        extended=[]
        for row,(vector,norm,coeff,key) in zip(rows,states):
            if len(row)!=137:raise ValueError('Old ledger row width differs')
            if key is None:
                s=int(next(iter(vector)));extra=[float(item[s].get(s,0)) for item in action[-2:]]
            else:
                z=np.array(coeff,float);extra=[float(z@np.array(matrix,float)@z)/norm for matrix in hop[key][-2:]]
            extended.append(np.r_[row[:-2],extra,row[-2:]])
        rows=extended
    if any(len(row)!=139 or not np.isfinite(row).all() for row in rows) or not np.isfinite(energies).all():raise ValueError('Finite139-row data required')
    ...
    arr=np.array(rows).T;rhs=[F(1)]+[F(0)]*136+[th,tj]
    result=linprog(energies,A_eq=arr[:137],b_eq=np.array(rhs[:137],float),A_ub=arr[137:],b_ub=np.array(rhs[137:],float),bounds=(0,None),method='highs',options={'dual_feasibility_tolerance':1e-10,'primal_feasibility_tolerance':1e-10})
    pricing=[]
    ...
    for iteration in range(args.pricing_rounds):
        ...
        bounds[137]=(bounds[137][0],min(0,bounds[137][1]));bounds[138]=(bounds[138][0],min(0,bounds[138][1]))
        ...
        if candidate_violation>2e-8 or max(dual[137:])>1e-10:raise ValueError('Numerical trust LP feasibility check failed')
        center=dual.copy()
        for key,a,ds,ph,q,ts,cols,k,derivatives in mats:
            ...
            price=a-np.einsum('i,ijk->jk',np.array(x[2:4],float)+dual[1:3],ds)-np.diag(dual[3:64]@ts)-np.einsum('i,ijk->jk',dual[64:137],h)-dual[137]*ph-dual[138]*q
            ...
            for column,value in enumerate(ev):
                ...
                v=vectors[:,column]/scale;coeff=[round(float(z/max(abs(v)))*10**6) for z in v]
        ...
        arr=np.array(rows).T
        result=linprog(energies,A_eq=arr[:137],b_eq=np.array(rhs[:137],float),A_ub=arr[137:],b_ub=np.array(rhs[137:],float),bounds=(0,None),method='highs',options={'dual_feasibility_tolerance':1e-10,'primal_feasibility_tolerance':1e-10})
    diagnostic={'new_vector_integer_scale':1000000,'trust_radius':args.trust_radius,'resume_ledger':str(args.resume_ledger),'source_sha256':{str(p.resolve().relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path(__file__),Path(__file__).with_name('coherent_projector_fraction_free.py'),args.resume_ledger,out/'profile_joint_r1_2_certificate.json']},'accepted':False,'proposal_written':False,'lp_tolerance':1e-10,'support_threshold':1e-14,'scale':args.scale,'extra_atom_file':str(args.extra_atoms),'extra_sampled_vectors':added_atoms,'pricing':pricing,'pricing_eigenvectors':args.pricing_eigenvectors,'pricing_radius':args.pricing_radius,'determinant_anchors':args.determinant_anchors,'states':len(states),'active_sectors':list(exact),'status':result.message,'seconds':time.monotonic()-started}
    ...
    if result.success:
        values=list(result.x)+list(np.array(rhs[137:],float)-arr[137:]@result.x);support=[i for i,v in enumerate(values) if v>1e-14];diagnostic.update(numerical_family_upper=float(result.fun)/5,basis_size=len(support))
        erows=[];eenergies=[]
        for index in support:
            if index>=len(states):erows.append([F(int(j==137+index-len(states))) for j in range(139)]);eenergies.append(F(0));continue
