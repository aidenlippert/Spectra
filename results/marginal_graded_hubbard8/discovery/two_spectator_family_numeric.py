...
def main():
    ...
    for key,a,ds,ph,q,ts,cols,k,derivatives in mats:
        ...
        directions=[np.zeros(116)]+[sign*np.eye(116)[j]*args.scale for j in range(116) for sign in (-1,1)]+[rng.normal(size=116)*args.scale for _ in range(16)]
        for delta in directions:
            _,vectors=eigh(m+np.einsum('i,ijk->jk',delta[:2],ds)+np.diag(delta[2:63]@ts)+np.einsum('i,ijk->jk',delta[63:116],h),subset_by_index=[0,0]);v=vectors[:,0]/scale;coeff=[round(float(z/max(abs(v)))*10**8) for z in v]
