"""Nonaccepting family proposal with bounded exact physical-column completion."""
...
def main():
    ...
    for iteration in range(args.pricing_rounds):
        ...
        arr=np.array(rows).T
        result=linprog(energies,A_eq=arr[:117],b_eq=np.array(rhs[:117],float),A_ub=arr[117:],b_ub=np.array(rhs[117:],float),bounds=(0,None),method='highs',options={'dual_feasibility_tolerance':1e-10,'primal_feasibility_tolerance':1e-10})
    diagnostic={'resume_ledger':str(args.resume_ledger),'source_sha256':{str(p.resolve().relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path(__file__),Path(__file__).with_name('one_column_physical_completion.py'),args.resume_ledger,out/'profile_joint_r1_2_certificate.json']},'accepted':False,'proposal_written':False,'lp_tolerance':1e-10,'support_threshold':1e-14,'scale':args.scale,'extra_atom_file':str(args.extra_atoms),'extra_sampled_vectors':added_atoms,'pricing':pricing,'pricing_eigenvectors':args.pricing_eigenvectors,'pricing_radius':args.pricing_radius,'determinant_anchors':args.determinant_anchors,'states':len(states),'active_sectors':list(exact),'status':result.message,'seconds':time.monotonic()-started}
    ...
    if result.success:
        ...
        try:
            matrix=[[row[i] for row in erows] for i in range(119)]
            try:weights=solve_basis(matrix,rhs)
            except ValueError:
                if len(support)!=118:raise
                nominees=[];candidate_energies={}
                for index, (vector,norm,coeff,key) in enumerate(states):
                    if key is not None or index in support:continue
                    row,energy=anchor(int(next(iter(vector))));nominees.append((index,row));candidate_energies[index]=energy
                for offset in range(2):
                    index=len(states)+offset
                    if index not in support:
                        nominees.append((index,[F(int(j==117+offset)) for j in range(119)]));candidate_energies[index]=F(0)
                index,weights,completion=complete(matrix,rhs,nominees)
                support.append(index);eenergies.append(candidate_energies[index]);completion.update(added_index=index,added_kind='physical_anchor' if index<len(states) else 'fidelity_slack');diagnostic['physical_completion']=completion
