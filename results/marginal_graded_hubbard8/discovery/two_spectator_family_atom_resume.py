"""Nonaccepting exact-basis proposal after adding rounded physical conic atoms."""
...
def main():
    ...
    atom_data=json.loads(args.extra_atoms.read_text());rng=np.random.default_rng(981737)
    atoms=sorted(atom_data['atoms'],key=lambda z:z['dual_weight'],reverse=True)
    atoms=[item for item in atoms[:16] if item['dual_weight']>1e-8]
    ...
    for atom in atoms:
        ...
        raw=np.array(atom['coordinates'],float)
        if raw.shape!=(len(cols),) or not np.isfinite(raw).all() or np.linalg.norm(raw)<1e-12:raise ValueError('Invalid atom coordinates')
        directions=[raw]+[raw+epsilon*rng.normal(size=len(raw))/np.sqrt(len(raw)) for epsilon in (1e-5,1e-4,1e-3) for _ in range(8)]
        for vector_raw in directions:
            v=vector_raw/scale;coeff=[round(float(z/max(abs(v)))*10**8) for z in v]
            vector={str(s):z*b for z,col in zip(coeff,cols) if z for s,b in col.items()};norm=sum(z*z for z in vector.values())
            w=np.array(coeff,float)*scale/np.sqrt(float(norm));d=np.array([w@matrix@w for matrix in ds])
            rows.append(np.r_[1,d,ts@(w*w),[w@matrix@w for matrix in h],w@ph@w,w@q@w]);states.append((vector,norm,coeff,key));energies.append(float(w@a@w-d@np.array(x[2:4],float)));added_atoms+=1
    if added_atoms>400:raise ValueError('400 additional physical candidates cap exceeded')
