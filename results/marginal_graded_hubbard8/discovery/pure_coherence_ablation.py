"""Untrusted frozen-coefficient ablation; remove only the two pure-coherence telescopes."""
from pathlib import Path
from fractions import Fraction as F
import argparse,hashlib,json,math,sys
from pure_coherence_frozen_target import ROOT,minimum
...
def main():
    ...
    if c.get('kind')!='hubbard_projector_extension_v19' or not c.get('pure_coherence'):
        raise ValueError('Explicit new-operator source required')
    c.pop('pure_coherence');c['kind']='hubbard_projector_extension_v18'
    ...
    files={Path(__file__).resolve(),args.seed.resolve(),Path(sys.modules['pure_coherence_frozen_target'].__file__).resolve()}
    ...
    result={'accepted':False,'numerical_minimum':value,'proposed_threshold':c['penalized_lower'],
            ...
            'scope':'Removed only pure_coherence and lowered schema to v18; all other coefficients fixed, scalar threshold reproposed. Requires exact replay.'}
