"""Verify exact frozen transfer and compare accepted two-spectator/ablated thresholds."""
...
def main():
    ...
    ap = BASE/'held_out/without_two/profile_joint_r1_2_certificate.json'
    ...
    expected_ablation.pop('two_spectator_hopping'); expected_ablation['kind']='hubbard_projector_extension_v14'
    ...
    original_previous=ROOT/'results/marginal_graded_hubbard8/pair_transfer/held_out/profile_joint_r1_2_certificate.json'
    ...
    result = {'accepted':True,'target':target,'with_two_spectator_open_lower':str(lower),'previous_recipe_open_lower':previous['lower_per_site'],'improvement_over_previous_recipe':str(lower-F(previous['lower_per_site'])),
              'without_two_spectator_open_lower':str(old_lower),'exact_improvement':str(lower-old_lower),
              'improvement_float':float(lower-old_lower),'physical_upper':energy['upper_per_site'],
              'source_sha256':{str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in files},
              'scope':'Frozen projector sources, penalties and correction coefficients transferred to a new U,V,W target on the same chain geometry. Only physical profiles and scalar local threshold changed. Comparison is between two accepted fixed-coefficient certificates; it is not a ceiling on a reoptimized family without two-spectator terms. No generic molecular, geometry or scalability claim.'}
