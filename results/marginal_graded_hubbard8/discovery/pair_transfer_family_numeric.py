...
def solve_basis(matrix,rhs):
    ...
    if not 1<=n<=m<=89 or any(len(row)!=n for row in matrix):raise ValueError('Bounded rectangular dual basis required')
...
def main():
    ...
    action=[actions()]+[spin_actions({label:1}) for label in LABELS]+[spectator_actions({label:1}) for label in SPECTATORS]+[pair_actions({label:1}) for label in PAIRS];hop={key:[projected_matrix(cols,item) for item in action] for key,a,ds,ph,q,ts,cols,*_ in mats}
    gamma=[F(c['hopping_telescope'])]+[F(c['spin_telescope'].get(label,0)) for label in LABELS]+[F(c['spectator_hopping'].get(label,0)) for label in SPECTATORS]+[F(c['pair_transfer'].get(label,0)) for label in PAIRS]
    ...
    for key,a,ds,ph,q,ts,cols,k,derivatives in mats:
        ...
        directions=[np.zeros(86)]+[sign*np.eye(86)[j]*args.scale for j in range(86) for sign in (-1,1)]+[rng.normal(size=86)*args.scale for _ in range(16)]
        for delta in directions:
            _,vectors=eigh(m+np.einsum('i,ijk->jk',delta[:2],ds)+np.diag(delta[2:63]@ts)+np.einsum('i,ijk->jk',delta[63:86],h),subset_by_index=[0,0]);v=vectors[:,0]/scale;coeff=[round(float(z/max(abs(v)))*10**8) for z in v]
    ...
    arr=np.array(rows).T;rhs=[F(1)]+[F(0)]*86+[th,tj]
    result=linprog(energies,A_eq=arr[:87],b_eq=np.array(rhs[:87],float),A_ub=arr[87:],b_ub=np.array(rhs[87:],float),bounds=(0,None),method='highs',options={'dual_feasibility_tolerance':1e-10,'primal_feasibility_tolerance':1e-10})
    pricing=[]
    ...
    for iteration in range(args.pricing_rounds):
        ...
        for key,a,ds,ph,q,ts,cols,k,derivatives in mats:
            scale=np.sqrt([sum(v*v for v in col.values()) for col in cols]);h=np.array(hop[key],float)/scale[:,None]/scale[None,:]
            price=a-np.einsum('i,ijk->jk',np.array(x[2:4],float)+dual[1:3],ds)-np.diag(dual[3:64]@ts)-np.einsum('i,ijk->jk',dual[64:87],h)-dual[87]*ph-dual[88]*q
    ...
    if result.success:
        values=list(result.x)+list(np.array(rhs[87:],float)-arr[87:]@result.x);support=[i for i,v in enumerate(values) if v>1e-14];diagnostic.update(numerical_family_upper=float(result.fun)/5,basis_size=len(support))
        erows=[];eenergies=[]
        for index in support:
            if index>=len(states):erows.append([F(int(j==87+index-len(states))) for j in range(89)]);eenergies.append(F(0));continue
