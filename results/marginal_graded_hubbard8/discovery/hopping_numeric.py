...
def main():
    ...
    for key,a,ds,ph,q,ts,cols,*_ in mats:
        scale=np.sqrt([sum(v*v for v in col.values()) for col in cols]);hop[key]=np.array(projected_matrix(cols,action),float)/scale[:,None]/scale[None,:]
    ...
    y0=np.array(list(map(float,x[2:4]))+[float(F(c['penalty'])),float(F(c['joint']['penalty']))]+taus+[float(F(c.get('signed_charge_telescope',{}).get(key,0))) for key in PATTERNS]+[float(F(c.get('hopping_telescope',0))),float(F(c['penalized_lower']))])
    assert len(y0)==67
    cost=np.array([0.,0.,float(theta_h)/5,float(theta_j)/5]+[0.]*62+[-.2]);cached=None;values=None;jac=None;best=None;calls=0
    def evaluate(y):
        ...
        for key,a,ds,ph,q,ts,*_ in mats:
            matrix=a+np.einsum('i,ijk->jk',y[:2]-np.array(x[2:4],float),ds)+y[2]*ph+y[3]*q+np.diag(y[4:-2]@ts)+y[-2]*hop[key]
            ev,vectors=eigh(matrix,subset_by_index=[0,0]);w=vectors[:,0]
            vals.append(float(ev[0])-y[-1]);grads.append([float(w@d@w) for d in ds]+[float(w@ph@w),float(w@q@w)]+list(ts@(w*w))+[float(w@hop[key]@w),-1.])
    ...
    outcome=None
    try:
        outcome=minimize(lambda y:float(cost@y),y0,jac=lambda y:cost,method='SLSQP',bounds=[(0,2.5)]*2+[(0,3)]*2+[(-2,2)]*62+[(-10,0)],constraints=[{'type':'ineq','fun':lambda y:evaluate(y)[0],'jac':lambda y:evaluate(y)[1]},{'type':'ineq','fun':lambda y:2.5-y[0]-y[1],'jac':lambda y:np.r_[-1.,-1.,np.zeros(65)]}],options={'maxiter':240,'ftol':1e-10})
    except RuntimeError as exc:
        if 'budget' not in str(exc):raise
    y=[F(round(float(value)*10**6),10**6) for value in best[1][:-1]]
    point=x.copy();point[2:4]=y[:2];fresh=physical(point);signed=dict(zip(PATTERNS,y[13:-1]));sparse={s:sum(t*shape.get(s,0) for t,shape in zip(y[4:13],shapes)) for s in set().union(*shapes)}
    fresh_action=actions()
    minimum=(float('inf'),None);difference=0.
    for key,a,ds,ph,q,ts,cols,*_ in mats:
        k=fresh[key][1];scale=np.sqrt([sum(v*v for v in col.values()) for col in cols]);direct=np.array(k,float)/scale[:,None]/scale[None,:]+float(y[-1])*np.array(projected_matrix(cols,fresh_action),float)/scale[:,None]/scale[None,:]
        ...
        affine=float(y[-1])*hop[key]+a+np.einsum('i,ijk->jk',np.array(y[:2],float)-np.array(x[2:4],float),ds)+np.diag(np.array(y[4:-1],float)@ts)
    ...
    ell=F(math.floor(minimum[0]*10**7)-1,10**7);out=dict(c,kind='hubbard_projector_extension_v11',hopping_telescope=str(y[-1]),penalty=str(y[2]),penalized_lower=str(ell),local_window=dict(c['local_window']),joint=dict(c['joint']))
    ...
    result={'accepted':False,'matrix_evaluations':calls,'optimizer_success':False if outcome is None else bool(outcome.success),'optimizer_message':'hard evaluation budget' if outcome is None else str(outcome.message),'proposed_periodic_lower':str(lower),'proposed_periodic_lower_float':float(lower),'max_fresh_affine_difference':difference,'hopping_telescope':str(y[-1]),'signed_charge_patterns':len(out['signed_charge_telescope']),'shape_indices':indices,'seconds':time.monotonic()-started,'source_sha256':{str(p.resolve().relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path(__file__),Path(__file__).with_name("signed_charge_numeric.py"),ROOT/"experiments/marginal_hopping_telescope.py",args.seed,candidates_path]},'scope':'Nonaccepting numerical search in all 52 disjoint signed-charge pattern directions and nine sparse shapes. Old diagonal profiles and compact corrections are fixed, since their variation is contained in the complete signed-charge span. Two hopping profiles, both penalties and one mean-zero range-three hopping telescope are optimized. Hard250 matrix evaluations; fresh physical reconstruction; exact PSD acceptance remains separate.'}
    (args.output/'hopping_proposal.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
