...
def main():
    ...
    if not r['accepted'] or c['kind']!='hubbard_projector_extension_v13' or r['source_sha256'][str(cp.relative_to(ROOT))]!=hashlib.sha256(cp.read_bytes()).hexdigest():raise ValueError('Accepted hopping energy certificate required')
    x,_,mats,half,hn,charged,cn,ratio=prepare(c,[])
    hopping_action=actions();gamma=F(c["hopping_telescope"])
    spin_action=spin_actions({key:F(value) for key,value in c["spin_telescope"].items()})
    ...
    for key,a,ds,ph,q,ts,cols,*_ in mats:
        d=np.array([float(sparse.get(next(iter(col))&1023,0)-sparse.get(next(iter(col))>>2,0)+signed_value(next(iter(col)),signed)) for col in cols]);scale=np.sqrt([sum(v*v for v in col.values()) for col in cols]);h=np.array(projected_matrix(cols,hopping_action),float)/scale[:,None]/scale[None,:];spin=np.array(projected_matrix(cols,spin_action),float)/scale[:,None]/scale[None,:];ev,vectors=eigh(a+float(gamma)*h+spin+np.diag(d)+float(alpha)*ph+float(beta)*q,subset_by_index=[0,0])
    ...
    without+=F(sum(a*b*vector.get(t,0) for s,a in vector.items() for t,b in spin_action[s].items()),norm)
    action=spectator_actions({key:F(value) for key,value in c['spectator_hopping'].items()});change=F(sum(a*b*vector.get(t,0) for s,a in vector.items() for t,b in action[s].items()),norm)
    ...
    result={'accepted':True,'sector':list(key),'integer_vector':{str(s):a for s,a in vector.items()},'vector_norm':norm,'without_spectator_residual':str(without),'without_spectator_residual_float':float(without),'with_spectator_residual':str(without+change),'with_spectator_residual_float':float(without+change),'source_sha256':{str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in sorted(files)},'scope':'Exact negative Rayleigh witness for the submitted local lower after deleting only the spectator hopping telescopes. Same vector has nonnegative residual with it. This proves necessity at these fixed certificate coefficients, not strict separation from the fully reoptimized older family.'}
    (folder/'spectator_ablation.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps({k:v for k,v in result.items() if k not in ['integer_vector','source_sha256']}))
