...
def main():
    ...
    if not receipt['accepted'] or not receipt.get('spin_telescope') or current['kind']!='joint_diagonal_range2_family_limit_v8' or previous['kind']!='joint_diagonal_range2_family_limit_v7':raise ValueError('Matched spin family and older hopping family required')
    ...
    for key in ['half_vector','charged_vector','ratio','theta_half','theta_joint','diagonal_shapes','W','range_two_density_profile']:
        if current[key]!=previous[key]:raise ValueError('Compared fixed families differ beyond added spin moments')
    if not energy.get('spin_telescope'):raise ValueError('Actual spin energy correction required')
