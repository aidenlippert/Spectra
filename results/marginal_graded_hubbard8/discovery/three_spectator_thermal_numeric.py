...
def main():
    ...
    action=[actions()]+[spin_actions({label:1}) for label in LABELS]+[spectator_actions({label:1}) for label in SPECTATORS]+[pair_actions({label:1}) for label in PAIRS]+[two_actions({label:1}) for label in TWO]+[three_actions({label:1}) for label in THREE];hop={}
    ...
    def evaluate(y):
        ...
        for key,a,ds,ph,q,ts,*_ in mats:
            matrix=a+np.einsum('i,ijk->jk',y[:2]-np.array(x[2:4],float),ds)+y[2]*ph+y[3]*q+np.diag(y[4:65]@ts)+np.einsum('i,ijk->jk',y[65:136],hop[key])
        ...
        if min(y[:4])>=0 and y[0]+y[1]<=2.5+1e-12 and max(abs(y[13:65]))<=8+1e-9 and max(abs(np.r_[y[4:13],y[65:136]]))<=2+1e-9 and (best is None or score>best[0]):best=(score,np.r_[y,minimum])
        return value,gradient
    point=y0[:-1].copy();initial_value,initial_gradient=evaluate(point)
    rng=np.random.default_rng(20260912);gradient_errors=[]
    for _ in range(2):
        direction=rng.normal(size=136);direction/=np.linalg.norm(direction);step=1e-8
    ...
    def coordinate_scale(y):
        ...
        errors=[]
        for j in (60,70):
            step=1e-9;direction=np.zeros(136);direction[j]=step
        ...
        direction=np.random.default_rng(20260912).normal(size=136);direction/=np.linalg.norm(direction);step=1e-9
    ...
    for tau in (1e-6,1e-7):
        ...
        try:
            ...
            bounds=np.array([(0,2.5)]*2+[(0,3)]*2+[(-2,2)]*9+[(-8,8)]*52+[(-2,2)]*71)
            def constraints(z):
            ...
            outcome=minimize(lambda z:evaluate(origin+transform@z)[0],np.zeros(136),jac=lambda z:transform.T@evaluate(origin+transform@z)[1],method='SLSQP',constraints=[{'type':'ineq','fun':constraints,'jac':lambda z:constraint_jac}],options={'maxiter':120,'ftol':1e-13})
    ...
    for key,a,ds,ph,q,ts,*_ in mats:
        matrix=a+np.einsum('i,ijk->jk',atom_point[:2]-np.array(x[2:4],float),ds)+atom_point[2]*ph+atom_point[3]*q+np.diag(atom_point[4:65]@ts)+np.einsum('i,ijk->jk',atom_point[65:136],hop[key])
    ...
    fresh_action=[actions()]+[spin_actions({label:1}) for label in LABELS]+[spectator_actions({label:1}) for label in SPECTATORS]+[pair_actions({label:1}) for label in PAIRS]+[two_actions({label:1}) for label in TWO]+[three_actions({label:1}) for label in THREE]
    minimum=(float('inf'),None);difference=0.
    for key,a,ds,ph,q,ts,cols,*_ in mats:
        k=fresh[key][1];scale=np.sqrt([sum(v*v for v in col.values()) for col in cols]);direct=np.array(k,float)/scale[:,None]/scale[None,:]+np.einsum('i,ijk->jk',np.array(y[65:136],float),np.array([projected_matrix(cols,item) for item in fresh_action],float))/scale[:,None]/scale[None,:]
        ...
        affine=np.einsum('i,ijk->jk',np.array(y[65:136],float),hop[key])+a+np.einsum('i,ijk->jk',np.array(y[:2],float)-np.array(x[2:4],float),ds)+np.diag(np.array(y[4:65],float)@ts)
