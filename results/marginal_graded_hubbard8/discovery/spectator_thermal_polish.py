...
def main():
    ...
    for tau in (1e-6,1e-7):
        before=calls
        try:
            outcome=minimize(lambda y:1000*evaluate(y)[0],point,jac=lambda y:1000*evaluate(y)[1],method='SLSQP',bounds=[(0,2.5)]*2+[(0,3)]*2+[(-2,2)]*80,constraints=[{'type':'ineq','fun':lambda y:2.5-y[0]-y[1],'jac':lambda y:np.r_[-1.,-1.,np.zeros(82)]}],options={'maxiter':120,'ftol':1e-10})
            point=outcome.x
    ...
    (args.output/'thermal_history.json').write_text(json.dumps({'stages':stages,'gradient_errors':gradient_errors,'soft_min_bound_checks':bound_checks,'best_unrounded_point':best[1].tolist(),'last_optimizer_point':point.tolist()},indent=2)+'\n')
