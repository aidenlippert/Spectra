...
def match(new, old, family):
    if new['kind'] != 'hubbard_projector_extension_v16' or old['kind'] != 'hubbard_projector_extension_v15':
        raise ValueError('Explicit three-spectator extension and preceding two-spectator certificate required')
...
def main():
    ...
    if not family_receipt.get('two_spectator_hopping'):
        raise ValueError('Preceding two-spectator family cap required')
    ...
    result = {'accepted':True,'strict_family_separation':lower>cap,'periodic_three_spectator_lower':str(lower),
              ...
              'scope':'Exact lower improvement at fixed target, size, projector sources, ratio, ceilings and sparse span. A positive signed separation proves the new lower exceeds the preceding full two-spectator family ceiling; a nonpositive value does not establish such separation. No enlarged-family optimality or physical interpretation of family ceilings.'}
