...
def main():
    ...
    assert len(point)==84 and tau>0
    spectra=[];minimum=float('inf')
    for key,a,ds,ph,q,ts,*_ in mats:
        matrix=a+np.einsum('i,ijk->jk',point[:2]-np.array(x[2:4],float),ds)+point[2]*ph+point[3]*q+np.diag(point[4:65]@ts)+np.einsum('i,ijk->jk',point[65:84],hop[key])
    ...
    atoms=[];mean=np.zeros(84);omitted=0.
