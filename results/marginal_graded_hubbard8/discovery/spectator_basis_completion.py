...
def main():
    ...
    n = len(support)
    if 85-n not in (1,2):
        raise ValueError('This completion run requires deficiency one or two')
    ...
    best = None
    tried = feasible = bounded = 0
    for indices in combinations(range(85),85-n):
        tried += 1
        if tried > 4000:
            raise ValueError('Completion enumeration cap exceeded')
        if len(indices) == 1:
            j = indices[0]
            denominator = transformed_old[n][j]
            numerators = [transformed_rhs[n]]
        else:
            j,k = indices
            a,b = transformed_old[n][j],transformed_old[n][k]
            cc,dd = transformed_old[n+1][j],transformed_old[n+1][k]
            denominator = a*dd-b*cc
            numerators = [transformed_rhs[n]*dd-b*transformed_rhs[n+1],a*transformed_rhs[n+1]-cc*transformed_rhs[n]]
        if not denominator:
            continue
        if denominator < 0:
            denominator = -denominator
            numerators = [-v for v in numerators]
        if min(numerators) < 0:
            continue
        current = [transformed_rhs[i]*denominator-sum(transformed_old[i][j]*v for j,v in zip(indices,numerators)) for i in range(n)]
        if d < 0:
            current = [-v for v in current]
        if min(current) < 0:
            continue
        feasible += 1
        cost_num = sum(v*e for v,e in zip(current,energies[:n])) + abs(d)*sum(v*energies[n+j] for j,v in zip(indices,numerators))
        cost_den = abs(d)*denominator*energy_scale*5
        if best is not None and cost_num*best['cost_den'] >= best['cost_num']*cost_den:
            continue
        raw = [F(v,abs(d)*denominator) for v in current] + [F(v,denominator) for v in numerators]
        columns = selected + [additions[j] for j in indices]
        weights = [v*item['norm'] for v,item in zip(raw,columns) if item['vector'] is not None]
        if any(len(str(v)) > 4096 or not 0 <= v <= 1 for v in weights):
            continue
        bounded += 1
        best = {'indices':indices,'raw':raw,'columns':columns,'cost_num':cost_num,'cost_den':cost_den}
    diagnostic = {'accepted':False,'proposal_written':False,'selected_columns':n,'deficiency':85-n,
                  ...
                  'pivot_bits':abs(d).bit_length(),'ledger_candidates':count}
    if best is not None:
