...
from three_spectator_fraction_free import solve as solve_basis
...
def main():
    ...
    action=[actions()]+[spin_actions({label:1}) for label in LABELS]+[spectator_actions({label:1}) for label in SPECTATORS]+[pair_actions({label:1}) for label in PAIRS]+[two_actions({label:1}) for label in TWO]+[three_actions({label:1}) for label in THREE];hop={key:[projected_matrix(cols,item) for item in action] for key,a,ds,ph,q,ts,cols,*_ in mats}
    gamma=[F(c['hopping_telescope'])]+[F(c['spin_telescope'].get(label,0)) for label in LABELS]+[F(c['spectator_hopping'].get(label,0)) for label in SPECTATORS]+[F(c['pair_transfer'].get(label,0)) for label in PAIRS]+[F(c['two_spectator_hopping'].get(label,0)) for label in TWO]+[F(c['three_spectator_hopping'].get(label,0)) for label in THREE]
    ...
    if list(map(F,ledger['rhs'])) != [F(1)]+[F(0)]*134+[th,tj]:raise ValueError('Resume moment target differs')
    ...
    if any(len(row)!=137 or not np.isfinite(row).all() for row in rows) or not np.isfinite(energies).all():raise ValueError('Finite119-row data required')
    ...
    arr=np.array(rows).T;rhs=[F(1)]+[F(0)]*134+[th,tj]
    result=linprog(energies,A_eq=arr[:135],b_eq=np.array(rhs[:135],float),A_ub=arr[135:],b_ub=np.array(rhs[135:],float),bounds=(0,None),method='highs',options={'dual_feasibility_tolerance':1e-10,'primal_feasibility_tolerance':1e-10})
    pricing=[]
    ...
    for iteration in range(args.pricing_rounds):
        ...
        bounds[135]=(bounds[135][0],min(0,bounds[135][1]));bounds[136]=(bounds[136][0],min(0,bounds[136][1]))
        ...
        if candidate_violation>2e-8 or max(dual[135:])>1e-10:raise ValueError('Numerical trust LP feasibility check failed')
        center=dual.copy()
        for key,a,ds,ph,q,ts,cols,k,derivatives in mats:
            ...
            price=a-np.einsum('i,ijk->jk',np.array(x[2:4],float)+dual[1:3],ds)-np.diag(dual[3:64]@ts)-np.einsum('i,ijk->jk',dual[64:135],h)-dual[135]*ph-dual[136]*q
        ...
        arr=np.array(rows).T
        result=linprog(energies,A_eq=arr[:135],b_eq=np.array(rhs[:135],float),A_ub=arr[135:],b_ub=np.array(rhs[135:],float),bounds=(0,None),method='highs',options={'dual_feasibility_tolerance':1e-10,'primal_feasibility_tolerance':1e-10})
    diagnostic={'new_vector_integer_scale':1000000,'trust_radius':args.trust_radius,'resume_ledger':str(args.resume_ledger),'source_sha256':{str(p.resolve().relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path(__file__),Path(__file__).with_name('three_spectator_fraction_free.py'),args.resume_ledger,out/'profile_joint_r1_2_certificate.json']},'accepted':False,'proposal_written':False,'lp_tolerance':1e-10,'support_threshold':1e-14,'scale':args.scale,'extra_atom_file':str(args.extra_atoms),'extra_sampled_vectors':added_atoms,'pricing':pricing,'pricing_eigenvectors':args.pricing_eigenvectors,'pricing_radius':args.pricing_radius,'determinant_anchors':args.determinant_anchors,'states':len(states),'active_sectors':list(exact),'status':result.message,'seconds':time.monotonic()-started}
    ...
    if result.success:
        values=list(result.x)+list(np.array(rhs[135:],float)-arr[135:]@result.x);support=[i for i,v in enumerate(values) if v>1e-14];diagnostic.update(numerical_family_upper=float(result.fun)/5,basis_size=len(support))
        erows=[];eenergies=[]
        for index in support:
            if index>=len(states):erows.append([F(int(j==135+index-len(states))) for j in range(137)]);eenergies.append(F(0));continue
