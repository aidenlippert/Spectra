...
def _replay(c, *, free_range_two_profile):
    """Shared exact constraints; the extra row has one fixed physical meaning."""
    ...
    if type(free_range_two_profile) is not bool:
        raise ValueError('Explicit bounded profile mode required')
    if type(mixture) is not list or not 1 <= len(mixture) <= 9+len(shapes)+int(free_range_two_profile):
