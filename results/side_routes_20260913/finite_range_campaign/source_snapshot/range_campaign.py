...
def search(task):
    ...
    def evaluate(short):
        ...
        score = hi-lo if objective == 'width' else hi
        calls += 1
    ...
    winner = min(candidates,key=lambda p:F(p['claim']['width' if objective=='width' else 'upper']))
...
def run(out, workers):
    ...
    tasks = [(name,r,obj,seed) for name in names for r in range(4)
             for obj in ('width','energy') for seed in (17,43)]
    searches = []
    ...
    transfer_tasks = []
    for name in names:
        for obj in ('width','energy'):
            for radius in range(4):
                eligible = [s for s in searches if s['case']==name and s['objective']==obj and s['radius']<=radius]
                winner = min(eligible,key=lambda s:F(s['winner']['claim']['width' if obj=='width' else 'upper']))
