...
def failure_state(label, timed_out, stderr):
    if timed_out or "envelope exceeded" in stderr:
        return "resource_obstruction"
    if "Proposed map changed an actual solver coefficient" in stderr:
        return "counterexample_to_encoded_claim"
    if any(name in stderr for name in ("ModuleNotFoundError", "ImportError", "SyntaxError", "NameError", "AttributeError")):
        return "implementation_failure"
    if label == "fresh_MPS" or label.startswith("solve_"):
...
class WorkflowFailure(RuntimeError):
    def __init__(self, label, result):
        self.stage = label
        self.scientific_state = failure_state(label, result["timeout"], Path(result["stderr"]).read_text())
        super().__init__(label + " failed: " + result["stderr"])
...
def campaign(output, method_path, primitive_path, specification=None, bond=16, seconds=50, selector_path=None):
    ...
    target = {"specification": json.loads((input_dir / "specification.json").read_text()),
              ...
              "constructor": "candidate-selected local/collective supports" if selector_path else "declared collective pair supports plus width-three local clusters",
              "selector_source_sha256": digest(json.loads(selector_path.read_text())["source"]) if selector_path else None,
    ...
    def command(label, arguments, seconds=120, stdlib=False):
        with ledger.measure(label) as cost:
            ...
            if result["exit_code"]:
                ...
                error = WorkflowFailure(label, result)
                cost["scientific_state"] = error.scientific_state
                raise error
        return result
    ...
    try:
        ...
        collective_clusters = []
        if selector_path:
            command("select_correlation_supports", ["research.rsi_discovery_20260916.v2.correlation_search", "worker",
                str(input_dir / "fixture.json"), str(selector_path.resolve()), str(output / "selected_supports.json")], 60, True)
            selected = json.loads((output / "selected_supports.json").read_text())
            clusters, collective_clusters = selected["clusters"], selected["collective_clusters"]
            ledger.append("correlation_supports_frozen", source_sha256=target["selector_source_sha256"],
                          fixture_sha256=digest(fixture), selected=selected)
        ...
        for mag in (0, 1):
            ...
            write(case / "design.json", {"clusters": clusters, "collective_pairs": True,
                "collective_clusters": collective_clusters,
        ...
        acceptance = {"error": str(error), "stage": getattr(error, "stage", None)}
        status = getattr(error, "scientific_state", "implementation_failure")
...
def main():
    ...
    if a.action == "prepare":
        ...
        campaign(a.output, a.method, a.primitive,
                 json.loads(a.specification.read_text()) if a.specification else None, a.bond, a.seconds, a.selector)
