...
def campaign(output, map_proposal, primitive_proposal, supplied=None):
    ...
    protocol = {"native_model": "gpt-6-astra", "effort": "high", "proposal_calls": 0 if supplied else 1, "model_deadline_seconds": 300,
        ...
        "supplied_proposal": {"path": str(supplied), "sha256": hashlib.sha256(supplied.read_bytes()).hexdigest(),
                              "scope": "unchanged program rerun under a restored runtime; previous failed stages stay charged"} if supplied else None}
    ...
    if supplied:
        ...
        ledger.append("supplied_proposal_imported", source_sha256=ledger.blob(proposal["source"]),
                      provenance=protocol["supplied_proposal"])
    else:
