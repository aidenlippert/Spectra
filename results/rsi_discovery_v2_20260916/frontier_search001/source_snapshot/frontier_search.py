...
def candidate_observables(source, data, state):
    ...
    payload = {"H": h, "D": d, "W": w, "Y": y, "modes": data["modes"],
               "particles": data["particles"], "max_additional_observables": 14, "cache": {}}
    ...
    try:
        if action == "construct":
            result = {"status": "proposal_constructed", "observables":
                      candidate_observables(request["source"], data, state)}
        else:
...
def campaign(source_inputs, output):
    ...
    env = environment({"fixture": digest(data), "state": digest(state)}, files, [])
    research = Research(ledger, {"claim": "Original robust H8 task: D <= -3/5", "amplitude_limit": "1/2"}, env)
    protocol = {"model": "gpt-6-astra", "effort": "high", "proposal_calls": 2,
    ...
    for index in range(2):
        ...
        if proposal:
            ...
            result = execute("construct", {"source": proposal["source"]}, label + "_observables", 120)
