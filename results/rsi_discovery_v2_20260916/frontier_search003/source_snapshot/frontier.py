...
from experiments.marginal_symbolic import canonical, decode, encode, product
...
def validate_observables(proposed, modes):
    if type(proposed) is not list or not 1 <= len(proposed) <= 14:
        raise ValueError("Propose between one and fourteen additional observables")
    checked = []
    for item in proposed:
        if type(item) not in (tuple, list) or len(item) != 2:
            raise ValueError("An observable is a real/imaginary polynomial pair")
        for poly in item:
            if type(poly) is not dict or len(poly) > 4096:
                raise ValueError("Observable polynomial exceeds the sparse representation envelope")
            for word, coefficient in poly.items():
                if type(word) is not tuple or len(word) > 4:
                    raise ValueError("Observable words must be degree<=4 tuples")
                if type(coefficient) not in (int, F):
                    raise ValueError("Observable coefficients must be exact rational numbers")
                if any(type(letter) is not tuple or len(letter) != 2 or
                       type(letter[0]) is not int or letter[0] not in (0, 1) or
                       type(letter[1]) is not int or not 0 <= letter[1] < modes for letter in word):
                    raise ValueError("Observable mode is outside the original Hamiltonian")
                if sum(2 * creation - 1 for creation, _ in word):
                    raise ValueError("This observable reduction requires particle-neutral words")
        real, imaginary = map(canonical, item)
        if canonical(adj(real)) != real or canonical(adj(imaginary)) != scale(imaginary, -1):
            raise ValueError("Observable must be exactly Hermitian")
        checked.append((real, imaginary))
    return checked


def construct(data, state, proposed=None):
    h, d, w, y = frozen_problem(data, state)
    extra = [w, y] if proposed is None else validate_observables(proposed, data["modes"])
    # The target and identity are fixed; candidate programs only supply extra observables.
    basis = [(mono(()), {}), d, *extra]
...
def replay(data, state, certificate, proposed=None):
    actual = construct(data, state, proposed)
