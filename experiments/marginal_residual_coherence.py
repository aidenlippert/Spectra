...
def build(label):
    if type(label) is not str or label not in LABELS:
        raise ValueError('Canonical residual-coherence label required')
    ...
    if not telescope or telescope != {key: value for key, value in expected.items() if value}:
        raise ValueError('Residual-coherence embedding identity failed')
    ...
    if F(max(rows.values()), denominator) > F(1, 4):
        raise ValueError('Fixed residual-coherence norm bound failed')
    return denominator, y, telescope


def coefficients(source):
    if type(source) is not dict or not 1 <= len(source) <= 2:
        raise ValueError('One or two residual-coherence components required')
    ...
    if not terms:
        raise ValueError('Nonzero exact residual-coherence telescope required')
    return terms
