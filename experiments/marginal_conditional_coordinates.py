"""Binomial physical-Q conditional moments for numerical proposal coordinates.

The transform is independently checked on sampled atom columns. Rank loss is
possible and must be checked by callers; full exact export remains mandatory.
"""
...
def conditional_map(self):
    ...
    maximum=0.;labels=MomentDictionary.labels(self);sample=labels[::max(1,len(labels)//40)]
    for label in sample:
    ...
    return T,{'conditional_rows':self.qrows,'conditional_transform_entries':T.size,
        'conditional_construction_seconds':time.monotonic()-start,'checked_columns':len(sample),
        'maximum_direct_count_error':maximum,
        'conditional_scope':'Binomial physical-Q test moments; no state enumeration. Numerical coordinates only. Full rank in the chosen invariant space and exact original export are required.'}
