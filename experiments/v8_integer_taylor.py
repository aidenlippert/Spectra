"""Conventional fraction-free Taylor proposer for the v8 cost baseline."""
...
def fraction_free_taylor(gen, initial, duration, tolerance, max_order=24):
    ...
    for k in range(max_order + 1):
        ...
        integer_cost['exported_fractions'] += len(c)+len(derivative)
        for grouping in ('l1', 'firstfit', 'weighted'):
            ...
            if bound <= tolerance:
                ...
                return Piece(duration, tuple(coeffs)), witness, dict(probes=probes, denominator=d, initial_denominator=q0, integer_work=integer_cost, generator=dict(gen.cost))
