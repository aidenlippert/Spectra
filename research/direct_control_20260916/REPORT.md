...
A separate extraction of the final bundle passed in 12.4464 seconds, with
47,022,080 bytes peak process memory. It reconstructed all three certificates,
the scoped obstruction and exact operator, passed nine standard-library tests,
and rejected all four deliberately corrupted claims. All 68 files in the
imported source archive still match the original archive byte for byte.

The finalized ledger currently contains 25 metered subprocesses totaling
208.8805 seconds, including the 90-second timeout. This remains the partial
process accounting described above, not a cold complete solver cost. The
portable proof bundle and repository accounting are separate artifacts.
