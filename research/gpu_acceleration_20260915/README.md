...
The final configuration uses the Python 3.12 environment for every stage and
adds `--fast-prepare --parallel-builds`. The former uses the existing exact
canonical-word spin projector during proposal construction; the latter runs
the nonsinglet and retained-map builds in separate processes with disjoint
output folders and JIT caches. The final accepting checker is unchanged.
Parallel stage durations overlap and must not be summed as elapsed time.

The tested state environment uses Python 3.12, quimb 1.15.0, NumPy 2.2.6,
SciPy 1.18.1, networkx and opt_einsum. The first matched CUDA comparisons use
