...
def run(case, state_python=None, fast_prepare=False, parallel_builds=False):
    case = case.resolve()
    ...
    protocol = dict(created_UTC=datetime.datetime.now(datetime.timezone.utc).isoformat(),
                    ...
                    parallel_nonsinglet_and_retained_preparation=parallel_builds,
    ...
    def execute(step):
        ...
        step_env = env.copy()
        if parallel_builds:
            step_env['NUMBA_CACHE_DIR'] = str(case/f'numba_cache_{name}')
        with (case/f'{name}.log').open('x') as log:
            try:
                result = subprocess.run(cmd, cwd=source, env=step_env, stdout=log, stderr=subprocess.STDOUT, timeout=timeout)
                status = result.returncode
            except subprocess.TimeoutExpired:
                status = 'timeout'
        row = dict(step=name, seconds=time.monotonic()-begin, exit_code=status)
        return row
    groups = [[step] for step in steps]
    if parallel_builds:
        groups = groups[:3]+[[steps[3],steps[4]]]+groups[5:]
    for group in groups:
        if len(group) == 1:
            batch = [execute(group[0])]
        else:
            # Both jobs read the fixed state/upper and write disjoint folders.
            # Separate JIT caches avoid even shared cache-file writes.
            with ThreadPoolExecutor(max_workers=2) as pool:
                batch = list(pool.map(execute, group))
        results.extend(batch)
        passed = all(row['exit_code'] == 0 for row in batch)
        (case/'execution.json').write_text(json.dumps(dict(steps=results, total_seconds=time.monotonic()-started,
                                                    completed=len(results)==len(steps) and passed,
                                                    parallel_builds=parallel_builds), indent=2)+'\n')
        for row in batch:
...
if __name__ == '__main__':
    ...
    p.add_argument('--parallel-builds', action='store_true')
    args = p.parse_args()
    run(args.case, args.state_python, args.fast_prepare, args.parallel_builds)
