...
## 7. Independent implementation audit finding

The local dense-matrix cross-check found a concrete conversion defect in the
current implementation: `expand_word` builds each expanded dictionary with a
dictionary comprehension. When two expansion paths produce the same Pauli or
eigen label, the later value overwrites the earlier value instead of summing
it. This occurs for multi-letter words (and can silently corrupt `ve`, hence
the commutator columns); an `n=2` comparison with `H = ZI + 2 IX + (1/2)XY`
and the `PP` column disagrees with direct `i[H,PP]` by a nonzero matrix term.
The expansion routine must accumulate collisions exactly before any recurrence
certificate can be considered sound. This is a root-module issue; the audit
tests deliberately expose it and do not weaken the assertion.
