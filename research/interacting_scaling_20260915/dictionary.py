...
def clustered_frame(modes, clusters, collective_pairs=True, complete=False, collective_clusters=()):
    import numpy as np
    ...
    collective_clusters = [tuple(c) for c in collective_clusters]
    if any(len(c) < 2 or len(set(c)) != len(c) or any(type(i) is not int or not 0 <= i < spatial for i in c) for c in clusters+collective_clusters):
    ...
    if complete:
        ...
        if collective_pairs or collective_clusters:
            ...
            supports = list(combinations(range(spatial), 2)) if collective_pairs else []
            cubic('collective_pair_supports', supports+collective_clusters)
        for index, cluster in enumerate(clusters):
            cubic(f'cluster_{index}', [cluster])
    return groups, specs


def ideal_basis(modes, clusters, complete=False, max_body=2):
    ...
    if max_body != 2:
        raise ValueError('Supported number multipliers have body order one or two')
    if complete:
