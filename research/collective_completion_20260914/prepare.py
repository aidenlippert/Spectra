...
def build(case,fixture,state,out,global_triples=False):
    ...
    if global_triples:
        ws=[tuple((0,i) for i in inds) for inds in combinations(range(m),3)];extra=[]
        for name,words in [('global-triples-',ws),('global-triples+',[dagger(w) for w in ws])]:
            parts={}
            for w in words:parts.setdefault(sig(w),[]).append(w)
            extra.extend({'name':name+str(k),'words':v} for k,v in sorted(parts.items()))
        first=next(i for i,g in enumerate(groups) if g['name'].startswith('local-triples'))
        groups=groups[:first]+extra+[g for g in groups[first:] if not g['name'].startswith('local-triples')]
    ...
    meta={'kind':'paired_quartic_v1','case':case,'global_triples':global_triples,'fixture':str(fixture),'state':str(state),
    ...
    rec={'case':case,'map_seconds':map_end-t,'moment_seconds':time.monotonic()-map_end,'total_seconds':time.monotonic()-t,
         ...
         'many_body_states_enumerated':0,'full_cubic_teacher_used':False,'three_particle_moments_constructed':False,'global_triples':global_triples}
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('case');p.add_argument('fixture');p.add_argument('state');p.add_argument('out');p.add_argument('--global-triples',action='store_true');a=p.parse_args()
    build(a.case,a.fixture,a.state,a.out,a.global_triples)
