...
def adaptive_with_square(gen,initial,duration,tolerance,*,trigger_ratio=F(3,2),term_cap=40,max_order=24,block_groups=None):
 ...
 for m in range(max_order+1):
  ...
  if tolerance<bound<=trigger_ratio*tolerance and len(raw)<=term_cap:
   ...
   if block_groups is not None:
    if type(block_groups) is not int or not 1<=block_groups<=16:raise ValueError('block group cap')
    original=proof['groups'];indices=sorted(range(len(original)),key=lambda i:(-F(original[i]['upper']),i))[:block_groups]
