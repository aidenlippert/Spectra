...
from research.rsi_discovery_20260916.v2.algebra import oracle, gram_reference, task_words, validate_map, map_cost
...
def capabilities_for(retained=None):
    """All systems have the existing monomial-map interface; only its backend differs."""
    def ordinary_map(payload):
        return gram_reference(payload["words"])
    capabilities = {"oracle": oracle, "monomial_map": ordinary_map}
    if retained:
        library = MethodLibrary(Ledger(retained["ledger"]))
        method = library.activate(retained["method_id"], {"oracle": oracle})
        def monomial_map(payload):
            cache = payload["cache"].setdefault("retained_map_" + retained["method_id"], {})
...
def evaluate(request):
    ...
    capabilities = capabilities_for(request.get("retained"))
    primitive_calls = {name: 0 for name in capabilities}
    for name, function in list(capabilities.items()):
        def counted(*args, _name=name, _function=function, **kwargs):
            primitive_calls[_name] += 1
            return _function(*args, **kwargs)
        capabilities[name] = counted
    ...
    for spec in request["tasks"]:
        ...
        for repeat in range(request.get("repeats", 3)):
            word_product.cache_clear()
            primitive_calls.update({name: 0 for name in primitive_calls})
            start = time.perf_counter()
            actual = candidate({"operators": [dict(p) for p in polys], "cache": {}})
            samples.append(time.perf_counter() - start)
            calls.append(dict(primitive_calls))
            start = time.perf_counter()
        ...
        records.append({"task": spec, "operator_count": len(polys),
            ...
            "cold_seconds": samples, "primitive_calls": calls,
            "median_seconds": statistics.median(samples), **map_cost(expected)})
    return {"status": "verified_encoded_claim", "claim": "exact Gram map of every supplied polynomial operator, including all cross terms",
            "tasks": records, "activation_seconds": activation,
            "backend": "acquired" if request.get("retained") else "ordinary",
            "universal_program_correctness_proved": False}
...
Every arm also has Spectra's existing monomial_map({'words': words, 'cache': cache}) capability.
Its exact output is canonical_word -> {(i,j): nonzero integer}, using the same diagonal and
symmetrized off-diagonal convention for the supplied monomial dictionary. The ordinary backend
uses the existing CAR word-product implementation; the acquired backend has the same contract.
Monomial-map cache storage must belong to payload['cache']; all candidate caches start empty.
...
The applicable implementation below is loaded automatically. The same base model, target
information and limits apply to every arm. The acquired method
...
After freezing, all algorithms will additionally run with the same ordinary monomial-map backend.
Inherited primitive execution speed by itself will not establish improved algorithm discovery.
